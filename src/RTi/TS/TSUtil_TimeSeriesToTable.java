...
/**
Indicate whether missing values in time series should be transferred as null (true) or the missing values transferred (false).
*/
private boolean __useNullForMissing = false;

/**
Constructor.
@param table Data table being filled with time series.
Column names need to have been defined but the table is expected to be empty (no rows).
...
@param includeMissingValues indicates whether missing values should be output in single-column output
(setting to false can be advantageous when processing sparse time series)
@param valueColumns column numbers (0+) corresponding to the correct column names for the time series data value(s);
if a single column is output, then the first array position will be used for each time series
@param flagColumns column numbers (0+) corresponding to the correct column names for the time series data flag(s);
if a single column is output, then the first array position will be used for each time series
...
@param useNullForMissing if true, use null in the table for missing values.
If false, transfer the time series missing value indicator (e.g., -999 or NaN).  These values are not universally handled.
*/
public TSUtil_TimeSeriesToTable (
	DataTable table,
	List<TS> tslist,
	int dateTimeColumn,
	int tableTSIDColumn,
	...
	DateTime outputEnd,
    DateTimeWindow outputWindow,
	boolean useNullForMissing ) {
    this.__table = table;
    this.__tsList = tslist;
    this.__dateTimeColumn = dateTimeColumn;
    this.__tableTSIDColumn = tableTSIDColumn;
    this.__tableTSIDFormat = tableTSIDFormat;
    this.__includeMissingValues = includeMissingValues;
    this.__valueColumns = valueColumns;
    this.__flagColumns = flagColumns;
    this.__outputStart = outputStart;
    this.__outputEnd = outputEnd;
    this.__outputWindow = outputWindow; // Allow null to speed performance checks
    this.__useNullForMissing = useNullForMissing;
...
public void timeSeriesToTable () {
    String routine = getClass().getSimpleName() + ".timeSeriesToTable";
    // Create a new list of problems.
    this.__problems = new ArrayList<>();

    if ( this.__tableTSIDColumn >= 0 ) {
        // Outputting single column table.
        // Iterate through each time series and dump the values within the requested range.
        TSIterator tsi = null;
        int valueColumn = this.__valueColumns[0];
        int flagColumn = -1;
        if ( (this.__flagColumns != null) && (this.__flagColumns.length == 1) ) {
            // Have a valid flag column.
            flagColumn = this.__flagColumns[0];
        }
        int setRow;
        int rowCount = -1; // Will align properly when incremented below.
        for ( TS ts : this.__tsList ) {
            if ( ts == null ) {
                continue;
            }
            try {
                tsi = ts.iterator(this.__outputStart,this.__outputEnd);
            }
            catch ( Exception e ) {
            ...
            while ( (tsdata = tsi.next()) != null ) {
                // Set the date.
                date = tsdata.getDate();
                if ( (this.__outputWindow != null) && !this.__outputWindow.isDateTimeInWindow(date) ) {
                    // Don't add the row.
                    continue;
                }
                // Check for missing.
                ...
                if ( isMissing && !this.__includeMissingValues ) {
                ...
                // Set the date/time.
                try {
                    this.__table.setFieldValue(setRow, this.__dateTimeColumn, new DateTime(date), true );
                }
                catch ( Exception e ) {
                    this.__problems.add ( "Error setting date " + date + " for row [" + setRow + "] (" + e + ").");
                }
                // Set the TSID.
                if ( this.__tableTSIDFormat == null ) {
                    ...
                    tsid = ts.formatExtendedLegend(this.__tableTSIDFormat);
                }
                try {
                    this.__table.setFieldValue(setRow, this.__tableTSIDColumn, tsid, true );
                }
                catch ( Exception e ) {
                    this.__problems.add ( "Error setting TSID " + tsid + " for row [" + setRow + "] (" + e + ").");
                }
                // Set the data value and optionally the flag.
                try {
                    if ( isMissing && this.__useNullForMissing ) {
                        this.__table.setFieldValue(setRow, valueColumn, null, true );
                    }
                    else {
                        // Set as a double because non-missing or missing and the missing value should be used.
                        this.__table.setFieldValue(setRow, valueColumn, Double.valueOf(value), true );
                    }
                }
                catch ( Exception e ) {
                    this.__problems.add ( "Error setting data value " + value +
                ...
                if ( flagColumn >= 0 ) {
                    ...
                    try {
                        this.__table.setFieldValue(setRow, flagColumn, flag, true );
                    }
                    catch ( Exception e ) {
                        this.__problems.add ( "Error setting data flag " + flag +
        ...
        if ( (this.__outputStart == null) || (this.__outputEnd == null) ) {
            // One or more of the requested dates is null so find the full period of the data.
            TSLimits limits = null;
            try {
                limits = TSUtil.getPeriodFromTS(this.__tsList, TSUtil.MAX_POR );
                if ( this.__outputStart == null ) {
                    outputStart = new DateTime(limits.getDate1());
                }
                if ( this.__outputEnd == null ) {
                    outputEnd = new DateTime(limits.getDate2());
                }
            }
            catch ( Exception e ) {
                // Worst case use the period from the first time series.
                outputStart = this.__tsList.get(0).getDate1();
                outputEnd = this.__tsList.get(0).getDate2();
            }
        }
        else {
            outputStart = new DateTime(this.__outputStart);
            outputEnd = new DateTime(this.__outputEnd);
        }
        // Run through the table to make sure that the date/times increment without gaps.
        int nrows = this.__table.getNumberOfRecords();
        Object o;
        ...
        int intervalMult = this.__tsList.get(0).getDataIntervalMult();
        if ( this.__outputWindow != null ) {
            // It is more efficient to get the first value in the output window and then see if there is a match in the table.
            dtWindowFirst = this.__outputWindow.getFirstMatchingDateTime(outputStart,outputEnd,intervalBase,intervalMult);
        }
        // Also figure out the first and last date from the data record block.
        for ( int irow = 0; irow < nrows; irow++ ) {
            try {
                o = this.__table.getFieldValue(irow, this.__dateTimeColumn);
                if ( o == null ) {
                    dtPrev = null;
                }
                else {
                    dt = (DateTime)o;
                    if ( (this.__outputWindow != null) &&  !this.__outputWindow.isDateTimeInWindow(dt) ) {
        ...
        if ( outOfOrderCount > 0 ) {
            ...
            // Cannot continue.
            this.__problems.add ( "Existing date/times in table are out of order.  Cannot add time series to table.  " +
                "If sorting the table, do it after all time series have been added." );
            return;
        }
        // If a data row was not found, then set it to the appropriate row,
        // if necessary padding with blank rows before or after the existing table rows to keep the time sequence intact.
        int setRow, setColumn; // Row and column for data set.
        int setRowFirst = -1; // First row to set data in.
        if ( dataRowFound < 0 ) {
            if ( this.__table.getNumberOfRecords() == 0 ) {
                ...
                if ( dtFirst.greaterThan(outputStart) ) {
                    ...
                    for ( dt = new DateTime(outputStart); dt.lessThan(dtFirst); dt.addInterval(intervalBase,intervalMult)) {
                        ...
                        try {
                            ...
                            this.__table.insertRecord(setRow,this.__table.emptyRecord(),false);
                            this.__table.setFieldValue(setRow, this.__dateTimeColumn, new DateTime(dt), true );
                            ++setRow;
                        }
                        catch ( Exception e ) {
                            this.__problems.add ( "Unexpected error initializing blank record for date " + dt + " (" + e + ").");
                    ...
                    // Below the time series values will be inserted to match the date/time.
                    setRow = this.__table.getNumberOfRecords() - 1;
        ...
        int tsListSize = this.__tsList.size();
        ...
        for (date = new DateTime(outputStart); date.lessThanOrEqualTo(outputEnd);
            ...
            try {
                o = this.__table.getFieldValue(setRow, this.__dateTimeColumn);
            }
            catch ( Exception e ) {
            ...
            if ( o == null ) {
                ...
                try {
                    ...
                    this.__problems.add ( "Error setting date " + date + " for col [" + this.__dateTimeColumn + "] row [" + setRow + "] (" + e + ").");
                    Message.printWarning(3,routine,e);
                }
            }
            else {
                ...
                if ( !date.equals((DateTime)o) ) {
                    this.__problems.add ( "Existing date/time in table row [" + setRow + "] (" + o + ") does not match time series (" + date +
            ...
            for ( its = 0; its < tsListSize; its++ ) {
                ...
                // Set the data value.
                if ( (this.__valueColumns == null) || (this.__valueColumns[its] < 0) ) {
                    // Unable to set column so skip.
                }
                else {
                    setColumn = this.__valueColumns[its];
                    try {
                        if ( ts.isDataMissing(value) && this.__useNullForMissing ) {
                            this.__table.setFieldValue(setRow, setColumn, null, true );
                            if ( Message.isDebugOn ) {
                            ...
                            // Set as a double because non-missing or missing and the missing value should be used.
                            this.__table.setFieldValue(setRow, setColumn, Double.valueOf(value), true );
                            if ( Message.isDebugOn ) {
                ...
                // Set the data flag.
                flagColumn = -1;
                if ( (this.__flagColumns != null) && (this.__flagColumns.length != 0) ) {
                    flagColumn = this.__flagColumns[its];
                }
                if ( flagColumn >= 0 ) {
                    // Column has been specified for flag so output.
                    flag = tsdata.getDataFlag();
