...
/**
The MonthTS class is the base class for monthly time series.
Derive from this class for specific monthly time series formats (override allocateDataSpace() to control memory management).
*/
...
/**
 * Data space for monthly values.  The dimensions are [year][month].
 */
private	double[][] _data;

/**
 * Data flags for each monthly value.  The dimensions are [year][month].
 */
protected String [][] _dataFlags;

/**
 * Minimum absolute month stored.
 */
protected int _min_amon;

/**
 * Maximum absolute month stored.
 */
protected int _max_amon;

/**
 * Use to return data without creating memory all the time.
 */
...
/**
Allocate the data space for the time series.
The start and end dates and the data interval multiplier must have been set.  Fill with the specified data value.
...
to the specified dates.  If the period is extended, missing data will be used to fill the time series.
...
throws TSException {
	...
	// To get the right data position, declare a temporary MonthTS with the old dates and save a reference to the old data.
...
public Object clone () {
	...
	// Clone the data (the actual values will be copied since they are primitives).
...
/**
Copy the data array from one time series to another.
The period from this time series instance is used for the copy.
@param ts The time series to copy the data from.
@param copy_missing If true, copy missing data.  If false, ignore missing data.
*/
...
At this time, monthly time series are always output in a matrix summary format.
...
<td>The precision of numbers as printed.
All data values are printed in a 9-digit column.  The precision controls how many digits are shown after the decimal.
</td>
<td>
...
protected int [] getDataPosition ( DateTime date ) {
	...
	// Don't care if data exists, but do care if dates are null.
...
The alternative of returning a null time series from a read method if no data are
available results in the header information being unavailable.
...
public void refresh () {
	...
	if ( limits.areLimitsFound() ) {
		// Reset the limits for the time series.
