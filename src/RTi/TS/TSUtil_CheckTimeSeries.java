...
/**
Check time series values against a specified criteria.
If a condition is met, matching cases can be output to a table.
*/
public class TSUtil_CheckTimeSeries
{

/**
List of notes generated by this command when criteria are met, guaranteed to be non-null.
*/
private List<String> checkNotes = new ArrayList<>();
...
*/
private CheckType checkCriteria = null;

/**
Start of analysis (null to analyze all).
*/
private DateTime analysisStart = null;

/**
End of analysis (null to analyze all).
*/
private DateTime analysisEnd = null;

/**
Starting date/time for analysis window, within a year.
*/
private DateTime analysisWindowStart = null;

/**
Ending date/time for analysis window, within a year.
*/
private DateTime analysisWindowEnd = null;

/**
Value as input to analysis, depending on checkType.
*/
private Double checkValue1 = null;

/**
Start of analysis (null to analyze all).
*/
private Double checkValue2 = null;

/**
Problem type, flag problem logging (default is check type).
*/
...
*/
private int checkCriteriaMetCount = 0;
...
*/
private String tableTSIDColumn = null;

/**
Column number for tableTSIDColumn.
*/
private int tableTSIDColumnNum = 4;

/**
Format of TSID for table TSID column.
*/
private String tableTSIDFormat = null;

/**
Name of table column for date/time.
*/
private String tableDateTimeColumn = null;

/**
Column number for tableDateTimeColumn.
*/
private int tableDateTimeColumnNum = -1;

/**
Name of table column for time series value.
*/
private String tableValueColumn = null;

/**
Precision of value column output.
*/
private int tableValuePrecision = -1;

/**
Column number for tableValueColumn.
*/
private int tableValueColumnNum = -1;

/**
Name of table column for time series flag.
*/
private String tableFlagColumn = null;

/**
Column number for tableFlagColumn.
*/
private int tableFlagColumnNum = -1;

/**
Name of table check type column for time series flag.
*/
private String tableCheckTypeColumn = null;

/**
Column number for tableCheckTypeColumn.
*/
private int tableCheckTypeColumnNum = -1;

/**
Name of table check message column for time series flag.
*/
private String tableCheckMessageColumn = null;

/**
Column number for tableCheckMessageColumn.
*/
private int tableCheckMessageColumnNum = -1;
...
public TSUtil_CheckTimeSeries ( TS ts, CheckType checkType,
    ...
    this.action = action;
    this.table = table;
    this.tableTSIDColumn = tableTSIDColumn;
    this.tableTSIDFormat = tableTSIDFormat;
    this.tableDateTimeColumn = tableDateTimeColumn;
    this.tableValueColumn = tableValueColumn;
    if ( tableValuePrecision < 0 ) {
    	this.tableValuePrecision = 4; // Default.
    }
    else {
    	this.tableValuePrecision = tableValuePrecision;
    }
    this.tableFlagColumn = tableFlagColumn;
    this.tableCheckTypeColumn = tableCheckTypeColumn;
    this.tableCheckMessageColumn = tableCheckMessageColumn;
...
private boolean checkTableSetup ( DataTable table, String tableTSIDColumn, String tableDateTimeColumn,
	...
	try {
		if ( (tableTSIDColumn != null) && !tableTSIDColumn.isEmpty() ) {
			this.tableTSIDColumnNum = table.getFieldIndex(tableTSIDColumn);
		}
	}
	catch ( Exception e ) {
		// Not found so create it.
		this.tableTSIDColumnNum = table.addField(0, new TableField(TableField.DATA_TYPE_STRING, tableTSIDColumn, -1, -1), "");
	}
	try {
		if ( (tableDateTimeColumn != null) && !tableDateTimeColumn.isEmpty() ) {
			this.tableDateTimeColumnNum = table.getFieldIndex(tableDateTimeColumn);
		}
	}
	catch ( Exception e ) {
		// Not found so create it.
		this.tableDateTimeColumnNum = table.addField(1, new TableField(TableField.DATA_TYPE_DATETIME, tableDateTimeColumn, -1, -1), "");
	}
	try {
		if ( (tableValueColumn != null) && !tableValueColumn.isEmpty() ) {
			this.tableValueColumnNum = table.getFieldIndex(tableValueColumn);
		}
	}
	catch ( Exception e ) {
		// The table column is not found so create it - use 4 digits of precision for the check output.
		this.tableValueColumnNum = table.addField(2, new TableField(TableField.DATA_TYPE_DOUBLE, tableValueColumn, -1, tableValuePrecision), "");
	}
	try {
		if ( (tableFlagColumn != null) && !tableFlagColumn.isEmpty() ) {
			this.tableFlagColumnNum = table.getFieldIndex(tableFlagColumn);
		}
	}
	catch ( Exception e ) {
		// Not found so create it.
		this.tableFlagColumnNum = table.addField(3,new TableField(TableField.DATA_TYPE_STRING, tableFlagColumn, -1, -1), "");
	}
	try {
		if ( (tableCheckTypeColumn != null) && !tableCheckTypeColumn.isEmpty() ) {
			this.tableCheckTypeColumnNum = table.getFieldIndex(tableCheckTypeColumn);
		}
	}
	catch ( Exception e ) {
		// Not found so create it.
		this.tableCheckTypeColumnNum = table.addField(4,new TableField(TableField.DATA_TYPE_STRING, tableCheckTypeColumn, -1, -1), "");
	}
	try {
		if ( (tableCheckMessageColumn != null) && !tableCheckMessageColumn.isEmpty() ) {
			this.tableCheckMessageColumnNum = table.getFieldIndex(tableCheckMessageColumn);
		}
	}
	catch ( Exception e ) {
		// Not found so create it.
		this.tableCheckMessageColumnNum = table.addField(5,new TableField(TableField.DATA_TYPE_STRING, tableCheckMessageColumn, -1, -1), "");
	}
	return true;
}
...
throws Exception {
    ...
    if ( !ts.hasData() ) {
        if ( checkCriteria == CheckType.MISSING ) {
            this.checkNotes.add ( "Time series " + tsid + " has no data." );
        }
        return;
    }
    ...
    if ( table != null ) {
    	doTable = checkTableSetup ( table, this.tableTSIDColumn, this.tableDateTimeColumn,
    		this.tableValueColumn, this.tableValuePrecision, this.tableFlagColumn, this.tableCheckTypeColumn,
    		this.tableCheckMessageColumn );
    	tableTSID = ts.formatLegend(this.tableTSIDFormat);
    }
    ...
    if ( checkCriteria == CheckType.REPEAT ) {
        ...
        if ( value1Int >= 2 ) {
            ...
            this.checkNotes.add ( "For check criteria " + CheckType.REPEAT + ", Value1 must be >= 2." );
        }
        return;
    }
    ...
    while ( (data = tsi.next()) != null ) {
        ...
        if ( message != null ) {
            ...
            this.checkNotes.add ( message );
            if ( this.flag != null ) {
                // Update the flag value.
                dataPoint = ts.getDataPoint(date, dataPoint );
                dataPoint.setDataFlag ( this.flag );
        ...
        if ( matchDetected && doTable ) {
    		...
    		if ( this.tableTSIDColumnNum >= 0 ) {
    			rec.setFieldValue(this.tableTSIDColumnNum, tableTSID);
    		}
    		if ( this.tableDateTimeColumnNum >= 0 ) {
    			// Make a copy since iterating.
    			rec.setFieldValue(this.tableDateTimeColumnNum, new DateTime(date));
    		}
    		if ( this.tableValueColumnNum >= 0 ) {
    			rec.setFieldValue(this.tableValueColumnNum, tsvalue);
    		}
    		if ( this.tableFlagColumnNum >= 0 ) {
    			rec.setFieldValue(this.tableFlagColumnNum, tsflag);
    		}
    		if ( this.tableCheckTypeColumnNum >= 0 ) {
    			rec.setFieldValue(this.tableCheckTypeColumnNum, this.problemType);
    		}
    		if ( this.tableCheckMessageColumnNum >= 0 ) {
    			rec.setFieldValue(this.tableCheckMessageColumnNum, message);
    		}
    		table.addRecord(rec);
        }
        ...
        if ( matchDetected ) {
        	...
        	if ( this.action != null )  {
	            ...
	            else if ( this.action.equalsIgnoreCase("SetMissing") ) {
	                ts.setDataValue(date, ts.getMissing() );
	            }
        	}
        }
    ...
    if ( checkCriteriaMetCount > 0 ) {
    	this.checkCriteriaMetCount = checkCriteriaMetCount;
	    if ( (this.flag != null) && !this.flag.isEmpty() ) {
	        // Remove leading + on flag, used to indicate concatenation.
	        String flag = StringUtil.remove(this.flag,"+");
	        if ( (this.flagDesc == null) || this.flagDesc.equals("") ) {
	            // Default description.
	            ...
	            message = "Detected " + checkCriteriaMetCount + " values where " + this.flagDesc + ".";
	            ts.addDataFlagMetadata(new TSDataFlagMetadata( flag, message));
	        }
	        // Add a message to the genesis since flags have been set.
	        ts.addToGenesis ( message + "  Set flag to " + this.flag + "." );
	    }
    }
}
...
throws Exception {
    ...
    if ( checkCriteria != CheckType.REPEAT ) {
        this.checkNotes.add ( "CheckTimeSeriesForward is only implemented for " + CheckType.REPEAT + ", logic problem." );
        return;
    }
    ...
    if ( (this.flag != null) && !this.flag.equals("") && (this.checkNotes.size() > 0) ) {
        // Remove leading + on flag, used to indicate concatenation.
        String flag = StringUtil.remove(this.flag,"+");
        if ( (this.flagDesc == null) || this.flagDesc.equals("") ) {
            // Default description.
            message = "Detected " + this.checkNotes.size() + " values where " + formatCriteriaForFlagDesc() + ".";
            ts.addDataFlagMetadata(new TSDataFlagMetadata( flag, message));
        }
        else {
            // Use supplied description.
            message = "Detected " + this.checkNotes.size() + " values where " + this.flagDesc + ".";
            ts.addDataFlagMetadata(new TSDataFlagMetadata( flag, message));
        }
        // Add a message to the genesis since flags have been set.
        ts.addToGenesis ( message + "  Set flag to " + this.flag + "." );
...
/**
Format the criteria for use in output, for example a time series data flag description.
@return a string that describes the criteria, suitable for the flag description.
*/
private String formatCriteriaForFlagDesc () {
    if ( this.checkCriteria == CheckType.ABS_CHANGE_GREATER_THAN ) {
        return "abs(change(value)) > " + StringUtil.formatString(this.checkValue1,"%.6f");
    }
    else if ( this.checkCriteria == CheckType.ABS_CHANGE_PERCENT_GREATER_THAN ) {
        return "percent(abs(change(value))) > " + StringUtil.formatString(this.checkValue1,"%.6f");
    }
    else if ( this.checkCriteria == CheckType.CHANGE_GREATER_THAN ) {
        return "change(value) > " + StringUtil.formatString(this.checkValue1,"%.6f");
    }
    else if ( this.checkCriteria == CheckType.CHANGE_LESS_THAN ) {
        return "change(value) < " + StringUtil.formatString(this.checkValue1,"%.6f");
    }
    else if ( this.checkCriteria == CheckType.IN_RANGE ) {
        return StringUtil.formatString(this.checkValue1,"%.6f") + " <= value <= " +
            StringUtil.formatString(this.checkValue2,"%.6f");
    }
    else if ( this.checkCriteria == CheckType.OUT_OF_RANGE ) {
        return "value < " + StringUtil.formatString(this.checkValue1,"%.6f") + " OR value > " +
        StringUtil.formatString(this.checkValue2,"%.6f");
    }
    else if ( this.checkCriteria == CheckType.MISSING ) {
        return "value is missing";
    }
    else if ( this.checkCriteria == CheckType.REPEAT ) {
        return "value repeats previous value";
    }
    else if ( this.checkCriteria == CheckType.LESS_THAN ) {
        return "value < " + StringUtil.formatString(this.checkValue1,"%.6f");
    }
    else if ( this.checkCriteria == CheckType.LESS_THAN_OR_EQUAL_TO ) {
        return "value <= " + StringUtil.formatString(this.checkValue1,"%.6f");
    }
    else if ( this.checkCriteria == CheckType.GREATER_THAN ) {
        return "value > " + StringUtil.formatString(this.checkValue1,"%.6f");
    }
    else if ( this.checkCriteria == CheckType.GREATER_THAN_OR_EQUAL_TO ) {
        return "value >= " + StringUtil.formatString(this.checkValue1,"%.6f");
    }
    else if ( this.checkCriteria == CheckType.EQUAL_TO ) {
        return "value = " + StringUtil.formatString(this.checkValue1,"%.6f");
    }
    else {
        throw new InvalidParameterException ( "Unrecognized check criteria \"" + this.checkCriteria + "\"" );
    }
}

/**
Return the analysis end date/time, or null if no analysis end is used.
...
/**
Return the analysis start date/time, or null if no analysis start is used.
...
/**
Return the analysis window end date/time, or null if no analysis window end is used.
@return the analysis window end date/time.
*/
public DateTime getAnalysisWindowEnd () {
    return this.analysisWindowEnd;
}

/**
Return the analysis window start date/time or null if no analysis window start is used.
...
/**
Return the check type being performed.
@return the check type being performed.
*/
...
/**
Return the list of check types that can be performed.
@return the list of check types that can be performed
*/
...
/**
Get the list of checks that can be performed.
@return the check type display names as strings.
*/
...
/**
Return the number of values where the check criteria were met.
@return the number of values where the check criteria were met
*/
public int getCheckCriteriaMetCount() {
	return this.checkCriteriaMetCount;
}
...
/**
Return a list of notes for the time series.
These are messages corresponding to matched analysis criteria.
@return the check notes list
*/
public List<String> getCheckNotes () {
