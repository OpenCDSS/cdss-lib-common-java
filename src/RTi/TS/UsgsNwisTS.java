...
{	String routine = "UsgsNwisTS.createTimeSeries";
	...
	TSIdent ident = null;
	try {	ident = new TSIdent ( site_no, agency_cd, datatype, "Day","");
	}
	catch ( Exception e ) {
	...
	else {	try {	ts = TSUtil.newTimeSeries ( ident.toString(), true );
		}
		catch ( Exception e ) {
			ts = null;
		}
...
{	BufferedReader in = null;
	...
	try {	in = new BufferedReader ( new InputStreamReader(
		...
		while( (string = in.readLine()) != null ) {
			if (	string.regionMatches(true,0,
				"# National Water Information System",0,35) ) {
				is_usgsnwis = true;
				break;
			}
...
{	String	routine = "UsgsNwisTS.readTimeSeries";
	...
	// USGS files do not have header information with the period for the
	...
	while ( (string = in.readLine()) != null ) {
		...
		if ( data_count == 0 ) {
			// Need to interpret information from the first line
			// to allocate the time series.  Some things are
			// hard-coded for now until we get a specification...
			units = "CFS";
			datatype = "Streamflow";
...
{	String	message, routine="UsgsNwisTS.writePersistent";
	...
	if (	!((data_interval_base == TimeInterval.DAY) &&
		(data_interval_mult == 1)) ) {
		message =
		"Currently, only 1Day time series can be saved as USGS NWIS";
		Message.printWarning( 2, routine, message );
		throw new IOException ( message );
	}
	...
	fp.println ( "#" );
	fp.println (
	"# THIS FILE WAS WRITTEN BY THE PROGRAM INDICATED BELOW");
	fp.println (
	"# (NOT THE USGS).  THE USGS FILE FORMAT HAS BEEN MAINTAINED FOR USE");
	fp.println (
	"# BY OTHER SOFTWARE");
	fp.println ( "#" );
	...
	for ( ; date.lessThanOrEqualTo(date2);
		...
		fp.println ( source + "\t" + id + "\t" +
		date.toString(DateTime.FORMAT_YYYY_MM_DD) + "\t" +
		StringUtil.formatString(tsdata.getData(), "%.0f") +
		"\t" + tsdata.getDataFlag() );
	}
	routine = null;
	id = null;
