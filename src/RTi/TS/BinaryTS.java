...
@param interval_base Time series interval base (see TimeInterval.*).  Currently
only monthly, daily, and hourly data are supported.
...
@param write_header Indicates if header should be written.  Use true when the
...
throws IOException
{	initialize ( tsfile, nts, interval_base, interval_mult, date1, date2,
			mode, write_header );
}
...
{	long pos = -1;
	...
	if ( parameter == PARAMETER_TS_ALIAS ) {
		...
		if ( Message.isDebugOn ) {
			Message.printDebug ( 1, "BinaryTS.calculatePosition",
			"Alias position [" + its + "] is " + pos );
		}
	}
		...
		if ( Message.isDebugOn ) {
			Message.printDebug ( 1, "BinaryTS.calculatePosition",
			"Description position [" + its + "] is " + pos );
		}
	}
		...
		if ( Message.isDebugOn ) {
			Message.printDebug ( 1, "BinaryTS.calculatePosition",
			"Has-header position [" + its + "] is " + pos );
		}
	}
...
to null.
@exception IOException if there is an error closing the file.
@exception SecurityException if there is an error deleting the file because of
a security problem.
*/
public void delete ()
...
{	int amon = date.getAbsoluteMonth();
	...
	if ( _interval_base == TimeInterval.MONTH ) {
		...
		pos += (((date.getDay() - 1)*24 +
			date.getHour())/_interval_mult)*8;
	}
...
@return the Vector position of the match or -1 if no match or the field is not
recognized.
*/
...
{	_tsfile = tsfile;
	...
	if ( Message.isDebugOn ) {
		Message.printStatus ( 1, "BinaryTS.intialize",
...
read.
@param pos Starting byte (currently only used to set the position after the
read).
...
BinaryTS file during processing.
@param its Time series index.
@exception Exception if the interval for the time series does not match that
...
time series header and data are available.
