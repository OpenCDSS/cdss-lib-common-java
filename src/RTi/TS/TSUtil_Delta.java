...
/**
 * Whether to copy the original time series data flags.
 */
private boolean __copyDataFlags = false;
...
*/
private DeltaValueType __deltaValueType = null;
...
Auto reset flag.
*/
private String __autoResetFlag = null;
...
/**
Rollover delta limit:
- to rollover, (ResetMax - delta) must be <= this
*/
private Double __rolloverDeltaLimit = null;

/**
Rollover flag.
*/
private String __rolloverFlag = null;

/**
Manual reset flag.
*/
private String __manualResetFlag = null;
...
@param copyDataFlags if true copy the data flags from the input time series before setting any other flags
@param deltaValueType the data value type for the delta (if null the default is the time series data value)
...
@param autoResetFlag flag for resetType=AUTO when a reset occurs, can specify as "Auto"
...
@param rolloverDeltaLimit the maximum (resetMax - valuePrev) to treat as a rollover (otherwise treated as a manual reset)
@param rolloverFlag flag for resetType=ROLLOVER when a rollover occurs, can specify as "Auto"
@param manualResetFlag flag for resetType=ROLLOVER when a manual reset occurs, can specify as "Auto"
...
*/
public TSUtil_Delta ( TS ts, DateTime analysisStart, DateTime analysisEnd, boolean copyDataFlags,
	DeltaValueType deltaValueType,
    TrendType trendType, ResetType resetType,
    String autoResetDatum, String autoResetFlag,
    Double resetMin, Double resetMax, Double rolloverDeltaLimit, String rolloverFlag, String manualResetFlag,
    Double resetProximityLimit, TimeInterval resetProximityInterval, String resetProximityFlag,
    ...
    this.__deltaValueType = deltaValueType;
    if ( deltaValueType == null ) {
    	// Default is the data value.
        this.__deltaValueType = DeltaValueType.DATA_VALUE;
    }
    ...
    this.__copyDataFlags = copyDataFlags;
    ...
    this.__autoResetFlag = autoResetFlag;
    if ( (this.__autoResetDatum == null) || this.__autoResetDatum.isEmpty() ) {
    	// Default is 0.
    	this.__autoResetDatumNumber = Double.valueOf(0.0);
    }
    ...
    this.__rolloverDeltaLimit = rolloverDeltaLimit;
    this.__rolloverFlag = rolloverFlag;
    this.__manualResetFlag = manualResetFlag;
    ...
    // Make sure that the rollover limit if specified is > 0 and of magnitude <= the range.
    if ( this.__rolloverDeltaLimit != null ) {
    	if ( this.__rolloverDeltaLimit <= 0.0 ) {
    		throw new InvalidParameterException ( "RolloverLimit must be > 0." );
    	}
    	if ( (this.__resetMin != null) && (this.__resetMax != null) ) {
    		if ( (this.__rolloverDeltaLimit > (this.__resetMax - this.__resetMin) ) ) {
        		throw new InvalidParameterException ( "RolloverLimit must be <= (ResetMax - ResetMin)." );
    		}
    	}
    }
...
throws Exception {
    ...
    // - units will match the original data (will be reset if the delta value is time)
    newts.copyHeader( ts );
    if ( this.__deltaValueType == DeltaValueType.TIME_SECONDS ) {
    	newts.setDataUnits("seconds");
    }
    else if ( this.__deltaValueType == DeltaValueType.TIME_MINUTES ) {
    	newts.setDataUnits("minutes");
    }
    else if ( this.__deltaValueType == DeltaValueType.TIME_HOURS ) {
    	newts.setDataUnits("hours");
    }
    else if ( this.__deltaValueType == DeltaValueType.TIME_DAYS ) {
    	newts.setDataUnits("days");
    }
    else if ( this.__deltaValueType == DeltaValueType.TIME_MONTHS ) {
    	newts.setDataUnits("months");
    }
    newts.setMissing(Double.NaN); // Default missing data value is NaN.
    newts.setDataType(newts.getDataType() + "-Delta" );
    if ( this.__analysisStart != null ) {
        newts.setDate1 ( this.__analysisStart );
    }
    if ( this.__analysisEnd != null ) {
        newts.setDate2 ( this.__analysisEnd );
    }
    if ( (this.__flag != null) && !this.__flag.equals("") ) {
    ...
    if ( this.__createData ) {
        ...
        if ( doResetRollover && (this.__flag != null) ) {
        	// Set the flag values and descriptions in the time series.
            if ( this.__flag.equalsIgnoreCase("Auto") ) {
                ...
                resetFlagHigh = this.__flag;
                resetFlagLow = this.__flag;
                valueFlagHigh = this.__flag;
                valueFlagLow = this.__flag;
            }
        }
        ...
        while ( (tsdata = tsi.next()) != null ) { // 'tsdata' is reused - don't use below when setting the flag.
            dt = tsdata.getDate();
           	// Get the data value from the time series:
            // - default is the data value
            // - if a time, ignore the time zone and set unused parts to zero
            // - then get the number of seconds from which to compute the longer intervals
            if ( this.__deltaValueType == DeltaValueType.TIME_SECONDS) {
            	value = 0.0;
            	LocalDateTime localDateTime = LocalDateTime.of(dt.getYear(), dt.getMonth(), dt.getDay(), dt.getHour(), dt.getMinute(), dt.getSecond() );
            	Instant instantUTC = localDateTime.toInstant(ZoneOffset.UTC);
            	value = instantUTC.getEpochSecond();
            }
            else if ( this.__deltaValueType == DeltaValueType.TIME_MINUTES) {
            	LocalDateTime localDateTime = LocalDateTime.of(dt.getYear(), dt.getMonth(), dt.getDay(), dt.getHour(), dt.getMinute(), 0 );
            	Instant instantUTC = localDateTime.toInstant(ZoneOffset.UTC);
            	value = instantUTC.getEpochSecond()/60;
            }
            else if ( this.__deltaValueType == DeltaValueType.TIME_HOURS) {
            	LocalDateTime localDateTime = LocalDateTime.of(dt.getYear(), dt.getMonth(), dt.getDay(), dt.getHour(), 0, 0 );
            	Instant instantUTC = localDateTime.toInstant(ZoneOffset.UTC);
            	value = instantUTC.getEpochSecond()/3600;
            }
            else if ( this.__deltaValueType == DeltaValueType.TIME_DAYS) {
            	value = dt.getAbsoluteDay();
            }
            else if ( this.__deltaValueType == DeltaValueType.TIME_MONTHS) {
            	value = dt.getAbsoluteMonth();
            }
            else {
            	// Default is to use the time series data value.
            ...
            // Can only compute the delta if the previous value and current value are not missing.
            ...
            if ( !ts.isDataMissing(valuePrev) && !ts.isDataMissing(value) ) {
            	...
            	if ( doResetAuto) {
                    ...
                    if ( this.__trendType == TrendType.DECREASING ) {
                        ...
                        */
                        if ( value > valuePrev ) {
                        	...
                        	// - delta is current value minus datum (datum) plus the value
                        	if ( debug ) {
                        		Message.printStatus(2, routine,
                        			"Increase detected for decreasing trend so resetting delta to (valuePrev - datum) + (value - datum).");
                        	}
                        	delta = (valuePrev - this.__autoResetDatumNumber) + (value - this.__autoResetDatumNumber);
                        }
                        else {
                    ...
                    else if ( this.__trendType == TrendType.INCREASING ) {
                    	// Expected trend is increasing so a negative change is a reset.
                        if ( value < this.__autoResetDatumNumber ) {
                        	// Value below the datum is not expected:
                        	// - allow but generate a problem message
                        	// - currently the datum is always 0
                            this.__problems.add( "Value " + value + " at " + dt
                            	+ " is < the auto reset datum (" + this.__autoResetDatum +
                            	").  Auto-resetting the delta to zero." );
                            delta = 0.0;
                            flag = getFlag (
        			            ...
        			            this.__autoResetFlag, // User supplied flag for rollover.
        			            "+A", // Default if "Auto" is specified.
                        	...
                        	if ( Message.isDebugOn ) {
                        		Message.printStatus(2, routine, "Decrease detected for increasing trend so reset to (value - datum).");
                        	}
                    ...
                    if ( this.__trendType == TrendType.DECREASING ) {
                        ...
                        if ( value > valuePrev ) {
                            ...
                            if ( valuePrev < resetMin ) {
                                // Previous value was less than the minimum so can't compute delta component on top
                                // of the previous value.
                                this.__problems.add( "Previous value " + valuePrev + " at " + dtPrev + " is < ResetMin " +
                                ...
                                flag = getFlag (
        			                "", // No flag is the global default.
        			                this.__rolloverFlag, // User supplied flag for rollover.
        			                "+R", // Default if "Auto" is specified.
        			                this.__flag ); // Global general flag.
                            }
                            // Now add the top part of the reset.
                            if ( value > resetMax ) {
                                this.__problems.add( "Value " + value + " at " + dt + " is > ResetMax " +
                    	...
                    	// Increasing trend.
                       	if ( value < valuePrev ) {
                       		...
                       		// - see how close the previous value was to the maximum to evaluate whether a rollover or manual reset
                       		double deltaPrev = this.__resetMax - valuePrev;
                       		if ( (this.__rolloverDeltaLimit != null) && (deltaPrev > this.__rolloverDeltaLimit) ) {
                    	 		// A manual reset is occurring.
                    	 		// - no delta is counted to ResetMax
                    	  		// - only count the delta from the value
                    	  		delta = value;
                           		isDeltaCalculated = true;
                           		flag = getFlag (
      			               		"", // No flag is the global default.
      			               		this.__manualResetFlag, // User supplied flag for manual reset.
       			               		"+r", // Default if "Auto" is specified.
       			               		this.__flag ); // Global general flag.
                    	 	}
                    	 	else {
                    	 		// A rollover is occurring.
                        		if ( debug ) {
                        			Message.printStatus ( 2, routine, "At " + dt + " delta computed inside reset.");
                        		}
                            	if ( valuePrev > resetMax ) {
                                	...
                                	// of the previous value.
                                	this.__problems.add( "Previous value " + valuePrev + " at " + dtPrev + " is > ResetMax " +
                            	...
                            	if ( value < resetMin ) {
                                	this.__problems.add( "Value " + value + " at " + dt + " is < ResetMin " +
            ...
            if ( isDeltaCalculated ) {
                ...
                if ( (flag != null) && !flag.isEmpty() ) {
                	...
                	if ( this.__copyDataFlags ) {
                		// Want to append new flag to the input time series flags.
                		newts.setDataValue(dt, delta, TSData.appendDataFlag(tsdata.getDataFlag(),flag), tsdata.getDuration());
                	}
                	else {
                		// Append/set the new flag and overwrite the existing flag.
                		newts.setDataValue(dt, delta, TSData.appendDataFlag("",flag), tsdata.getDuration());
                	}
                }
                else {
                    // No new flag to append/set.
                	if ( this.__copyDataFlags ) {
                		newts.setDataValue(dt, delta, tsdata.getDataFlag(), tsdata.getDuration());
                	}
                	else {
                		// No flags are set.
