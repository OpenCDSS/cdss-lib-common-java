...
private void fillDependentTS ()
{
    ...
    RegressionErrors regressionErrors = null;
        ...
        try {
            ...
            if ( tsToFill.isDataMissing(tsToFill.getDataValue(date)) ) {
                ...
                if ( !regressionChecksMask[month - 1] ) {
                    ...
                    //    analysis.formatInvalidRelationshipReason(regressionData,regressionChecks,regressionErrors) + ").");
                    continue;
                }
    ...
    if ( fillCount > 0 ) {
        tsToFill.addToGenesis ( "Filled " + fillCount + " missing values " + fillStart + " to " + fillEnd + " using analysis results:" );
...
{   String routine = getClass().getName() + ".saveStatisticsToTable";
    ...
    // Statistics from the filled data (include for comparison with mixed station analysis)...
    statsToOutput.add ( "NY_filled" );
    statsToOutput.add ( "MeanY_filled" );
    statsToOutput.add ( "SY_filled" );
    statsToOutput.add ( "SkewY_filled" );
    ...
    TSRegressionErrors tsRegressionErrors = analysis.getTSRegressionErrors();
    ...
    for ( int iEquation = 1; iEquation <= numEquations; iEquation++ ) {
        for ( int iStatistic = 0; iStatistic < statsToOutput.size(); iStatistic++ ) {
            ...
            if ( statisticName.equals("a") ) {
                ...
                if ( numEquations == 1 ) {
                    statisticValueDouble[countStatistic] =
                        tsRegressionErrors.getSingleEquationRegressionErrors().getMeanY1Estimated();
                }
                else {
                    statisticValueDouble[countStatistic] =
                        tsRegressionErrors.getMonthlyEquationRegressionErrors(iEquation).getMeanY1Estimated();
                }
            }
                ...
                if ( numEquations == 1 ) {
                    statisticValueDouble[countStatistic] =
                        tsRegressionErrors.getSingleEquationRegressionErrors().getRMSE();
                }
                else {
                    statisticValueDouble[countStatistic] =
                        tsRegressionErrors.getMonthlyEquationRegressionErrors(iEquation).getRMSE();
                }
            }
                ...
                if ( numEquations == 1 ) {
                    statisticValueDouble[countStatistic] =
                        tsRegressionErrors.getSingleEquationRegressionErrors().getStandardErrorOfEstimate();
                }
                else {
                    statisticValueDouble[countStatistic] =
                        tsRegressionErrors.getMonthlyEquationRegressionErrors(iEquation).getStandardErrorOfEstimate();
                }
            }
