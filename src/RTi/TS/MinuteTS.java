...
@param retain_previous_values If true, the array size will be increased if necessary,
...
throws Exception {
	...
	for ( i = 0; i < nmonths; i++, date.addMonth(1) ) {
		...
		for ( j = 0; j < ndays_in_month; j++ ) {
			if ( i == 0 ) {
				// In the first month.  If the day is less than the first day in the period, do not use up memory.
				day = j + 1;
				...
				// In the last month.  If the day is greater than the last day in the period, do not use up memory.
				day = j + 1;
...
The beginning and ending dates and interval multiplier must have been set.
...
public int allocateDataSpace( ) {
	...
	for ( imon = 0; imon < nmonths; imon++, date.addMonth(1) ) {
		...
		// Now allocate the memory for the number of intervals in the day.
		// Need to do this to save on memory:  if a day is not in the period,
		// skip this step but do allocate the full day for any day that does occur in the period.

		for ( iday = 0; iday < ndays_in_month; iday++ ) {
			if ( imon == 0 ) {
				// In the first month.  If the day is less than the first day in the period, do not use up memory.
				// The position will never be accessed.
				day = iday + 1;
				if ( day < _date1.getDay() ) {
					continue;
				}
...
@return The number of data points for a minute time series given the data interval multiplier for the specified period.
    	...
    	for ( int imon = 0; imon < _data.length; imon++ ) { // Months.
    		...
    		for ( int iday = 0; iday < _data[imon].length; iday++ ) { // Days in month.
    			if ( _data[imon][iday] == null ) {
    				// Memory is not allocated for days at the beginning and ends of months (outside the period),
    				// in order to save memory and increase performance.
    				continue;
    			}
...
At this time, minute time series are always output in a matrix summary format.
...
it is advised that if formatting is to remain the same over time,
...
throws TSException {
	...
	// Only know how to do this for 24-hour time series
...
The output is a simple format with YYYY-MM-DD HH on the left and then the minutes filling out the row.
If 1 minute data, two rows of values are used (might need to change later but 1 minute is unlikely).
Otherwise, a single row is used.  The right column is the total for the hour.
The data values are always a maximum of 9 characters.
...
private void formatOutputNMinute ( List<String> strings, PropList props,
	...
	for ( ; date.lessThanOrEqualTo(end); date.addInterval(_data_interval_base,_data_interval_mult) ) {
		...
		// The last data value for the day will be in numcol (e.g. 30 for 1minute).
		// The above will then have incremented the value to 31.
		if ( col > numcol ) {
...
If non-null, the provided instance will be used (this is often desirable during iteration to
...
private int [] getDataPosition ( DateTime date ) {
	...
	// Calculate the interval position of the data.
	// Know that minute data are stored in a 3D array with the last dimension being the minute data by interval.
	// Note that the recording at 00:00 of the current day is the first reading of the day.
...
Instead, return a TS with only the header information and call hasData()
to check to see if the data space has been assigned.
