...
public TSUtil_ChangeInterval ( TS oldTS, TimeInterval newInterval,
    ...
    if ( tolerance == null ) {
        tolerance = Double.valueOf ( 0.01 ); // Default.
    }
    ...
    if ( allowMissingCount == null ) {
        allowMissingCount = Integer.valueOf(0); // Default is don't allow missing.
    }
    // If the given value is less than 0, throw exception.
    ...
    if ( allowMissingConsecutive == null ) {
        if ( allowMissingCount > 0 ) {
            ...
            allowMissingConsecutive = Integer.valueOf(allowMissingCount);
        }
        else {
            allowMissingConsecutive = Integer.valueOf(0); // Default is don't allow missing.
        }
    }
    // If the given value is less than 0, throw exception.
        ...
        while ( needsAdjustment && iterations < 15 ) {
            ...
            while ( !lastDateNew.greaterThan(newTS.getDate2())) {
                ...
                if ( oldTS.isDataMissing(oldValue)) {
                    ...
                    // First and last values are averaged currentDateNew collects middle values.
                    ...
                    if ( (Math.abs(simulatedMean - oldValue) / oldValue) > tolerance ) {
                        ...
                        // ENDING ENDPOINT: rather than setting the last data value like the others,
                        // average the new adjusted value with the old (not from the old TS but the old simulated value).
                        tmp = newTS.getDataValue(currentDateNew);
     ...
     * This is the main routine which will call subordinate routines, depending on the intervals that are involved.
    ...
    throws Exception {
        ...
        // a single interval of the other for conversion that does not involve IRREGULAR time series.
        // If the old interval is smaller than the newer interval the intervalRelation is negative.
        // If the old interval is greater than the new interval the intervalRelation is positive.
        // If the intervals are not multiple of each other than the intervalRelation is 0.
        // If the older time series is IRREGULAR the intervalRelation is -1.
        // Cases where both time series are IRREGULAR or the newer time series is IRREGULAR should not happen because they
        ...
        if ( (newtsBase == TimeInterval.YEAR) && (outputYearType != YearType.CALENDAR) ) {
            ...
            // -------------------------------------------------------------
            // From IRREGULAR ACCM, MEAN and INST input TS use routine changeInterval_fromIRREGULAR
            ...
            if ( (oldTimeScale == TimeScaleType.MEAN) || (oldTimeScale == TimeScaleType.ACCM) ) {
                // -----------------------------------------------------
                // From REGULAR ACCM and MEAN input TS use routine changeInterval_toMEANorACCM.
        ...
        if (intervalRelation < 0) {
            ...
            for (; newTSi.next() != null;) {
                ...
                while (oldData != null && oldTSi.getDate().lessThanOrEqualTo(newTSi.getDate())) {
                    ...
                    // Should carry forward on input be allowed (seems to be implied by replaceDataValue() call below.
                    if ( statistic == null ) {
    ...
    private boolean changeIntervalFromInstByInterpolation(TSIterator oldTSi, TSIterator newTSi,
        ...
        for (; oldTSi.next() != null;) {
            ...
            // Save the first point (missing or previousValue) and advance to the next.
            ...
            if (previousValueMissing || currentValueMissing) {
                ...
                // Loop through the new TS until the end of the old interval - 1 new interval,
                // computing and saving the interpolated values.
                while (newTSi.getDate().lessThan(oldTSi.getDate())) {
    ...
    private boolean changeInterval_fromIRREGULAR(TSIterator oldTSi, TSIterator newTSi,
        ...
        // While Loop.
        while (true) {
            // TODO [LT] 2005-03-06 - This is one of the possible location for the code to make this method general.
            ...
            if (oldData != null && oldDate.equals(newDate, highestPrecision)) {
                ...
                if (first) {
                    // Do not set data at the first time interval do not know for which time scale it was valid.
                    first = false;
                }
                else {
            ...
            // Have to fill in one or more regular 'new' data points in this irregular 'old' interval.
            // TODO [LT] Does this EqualTo ever get executed, considering the equals() test above?
                ...
                if (first) {
                    ...
                    // Find out how many regular intervals will be in the irregular interval.
                    // This returns 1 even when less than one interval fits between the dates!!
                    ...
                    // double considering the real number of intervals from the previous oldTsDate to the oldDate.
                    // Doing the way it is done here it is looking like we are losing information every time we move from one
                    // old data point to the other, when the new data point is in between.
                    // ??????????????????????????????????????????????????????

                    // Write to the new regular TS: this is only executed when more than one new interval in old interval.
                    for (int i = 0; i < noNewDataPoints; i++) {
                        switch (handleMissingInputHow) {
                        case SET_TO_ZERO: // ALERT_INCREMENT.
                        case REPEAT: // ALERT_REGULAR.
                            // Take care of ALERT type data set to respective 'missing' value if dates are not equal.
                        ...
                        case KEEP_MISSING:
                            ...
                            default:
                                ...
                                if (previousOldTsValue != oldMissing) {
                                    ...
                                    // ??????????????????????????????????????????????????????
                                    // TODO [LT] 2005-02-28 We may need to have something considering the real number of
                                    // intervals from the previous oldTsDate to the oldDate.
                                    // Doing the way it is done here it is looking like we may lose information when moving from one
                                    // old data point to the other, when the new data point is in between.
                                    // Here we interpolate from the previousOldTsValue, using the noNewDataPoints that was computed from
                                    // the newDate which may not coincide with the date of the previousOldTsValue.
                                    // ??????????????????????????????????????????????????????
                        ...
                        // Have an offset in the interval dates (i.e. oldDate = 1:30, newDate = 1:35) so use the last old value.
                        else {
                            if ( (handleMissingInputHow == TSUtil_ChangeInterval_HandleMissingInputHowType.SET_TO_ZERO) ||
                                (handleMissingInputHow == TSUtil_ChangeInterval_HandleMissingInputHowType.REPEAT) ) {
                                // ALERT_INCREMENT and ALERT REGULAR.
                                incrementDate = false;
                                // newTSValue = oldTSValue;
                            }
                            else {
            ...
            // Here the oldTSdate is smaller than the newTSDate - have to work on the old data.
            // TODO [LT] Does this EqualTo ever get executed, considering the equals() test above the previous test?
            else {
                ...
                // Loop through the old irregular intervals until the current oldDateTime is larger or equal to the newDateTime.
                sum = 0.0;
                int i = 0;
                ...
                // ?????????????????????????????????????????????????????
                // TODO [LT] 2005-02-28 Here we should find out the proportional value for ACCM,
                // MEAN anD the interpolation interval.
                // newTSValue = sum/(iLeft + i + iRight);
                // ?????????????????????????????????????????????????????

                // Now the oldDateTime is larger than the newDateTime process the old data in this interval.

                if (i > 0) {
                    // Means have non-missing old data.
                    ...
                    // There is no old data for this new TS interval.
                	// The value for this new TS data point would be missing, but we allow it to be changed, if ALERT type data is used.

                    // Replace missing data value:
                    // - by 0 if ALERT INCREMENT data
                    // - by lastNewTSValue if ALERT REGULAR
                    // - by newMissing if REGULAR data
                    if ( (handleMissingInputHow == TSUtil_ChangeInterval_HandleMissingInputHowType.KEEP_MISSING)||
                        (handleMissingInputHow == TSUtil_ChangeInterval_HandleMissingInputHowType.REPEAT)||
                        (handleMissingInputHow == TSUtil_ChangeInterval_HandleMissingInputHowType.SET_TO_ZERO) ) {
                        ...
                        // By default with missingValueFlag 0 or -1,
                    	// the data is replaced by the 3rd parameter which is newMissing.
    ...
    private boolean changeInterval_toINST(TSIterator oldTSi, TSIterator newTSi, int intervalRelation,
        ...
        if (intervalRelation > 0) {
            ...
            // To clarify which TS each variable is referring to, variables will either start with "old" or "new"
        	// so the variable oldCurrentValue isn't as strange as it sounds - it's just the currentValue of the old TS.
            TS newTS = newTSi.getTS();
            TS oldTS = oldTSi.getTS();
            int oldIntervalBase = oldTS.getDataIntervalBase();
            int oldIntervalMult = oldTS.getDataIntervalMult();
            ...
            // Just in case we have just one value in the old time series,
            // the loop below will not run, so we need to say that this
            // data point is the current one, instead of the previous,
            // to allow the code after the loop to save this single point.
            ...
            while (!oldTSi.isIterationComplete() ) {
                ...
                // Can't be trying to assign data to newTS past it's end-point.
                ...
                if (oldCurrentValueMissing) {
                    ...
                    // This works whether are on a continuing rise or fall.
                    ...
                    // This works whether are on a peak or trough.
                    startValue = oldCurrentValue - ( .25 * ( oldCurrentValue - oldPreviousValue ));
...
private void changeIntervalToDifferentYearType ( TS oldTS, TS newTS,
    ...
    // newTSi should be set to the start of the new time series start date.
	// After routine has run, it will be set at or before nextDate.
	// Additionally, all checks have been done by this point to ensure no missing data is present.
    ...
    // newTSi should be set to the start of the new time series start date.
    // After routine has run, it will be set to nextDate.
    // Additionally, all checks have been done by this point to ensure no missing data is present.
   ...
   throws Exception {
        ...
        // Declaring time series, interval base and interval multipliers for the newTS and the oldTS time series.
        TS newTS = newTSi.getTS();
        ...
        // Define if the old time series values are time stamped at the end of the interval ( interval < DAY ).
        // If the abs(intervalRelation) is 1, are converting the old time series to a new time series of the same interval.
        // In this case we should not time stamp the new data at the beginning.
        boolean oldTsTimeStampedAtEnd = true;
        // Valid only for ACCM and MEAN when the time step is < DAY and if not converting between the same interval.
        ...
        // Define if the new time series values should be time stamped at the end of the interval ( interval < DAY ).
        // If the abs(intervalRelation) is 1, we are converting the old time
        // series to an new time series of the same interval.
        // In this case we should not time stamp the new data at the beginning.
        ...
        // Quality flag is set to S (no missing values) or (M) if missing values were found and allowed.
        ...
        if (intervalRelation < 0) {
            ...
            // This reseting of the oldTsTimeStampedAtEnd to FALSE if the input time scale is INST,
        	// should only be used when the input interval is shorter than the output interval.
            if (oldTimeScale == TimeScaleType.INST) {
                oldTsTimeStampedAtEnd = false;
            }
            ...
            for (; newTSi.next() != null;) {
                ...
                // If the old time series is < DAY (stamped in the end) need to iterate until the end of the period,
                // which is the new ts current data point. If the old time
                // series is >= DAY, we only have to iterate until the
                // old data point prior the the current new date.

                // Variable newTS_endDate valid only when the oldTS time interval is < DAY and is not ACCM or MEAN.
                DateTime newTS_endDate = new DateTime(newTSi.getDate());

                // Update the variable newTS_endDate if the oldTS time interval is >= DAY or in any case if INST.
                if (!oldTsTimeStampedAtEnd) {
                    newTS_endDate.addInterval(oldTSBase, -oldTSMult);
                }

                // This variable is used to allow counting of missing values (in the old time series) past the end of the
                // old time series, if the end of the old time series does not coincide with the new time series data point.
                ...
                // If the new time series is < DAY (stamped at the end) need to assign the value to the new time series current data point.
                // If the new time series is >= DAY (stamped at the beginning), need to assign the value to the previous new date.
                ...
                // If the oldTSiPreviousDate is not equal to the newTSi.getDate() that is because we are processing
                // the last interval in the period and the old time series data terminate short than the new time series date.
                // If the end of the old time series coincide with the new time series data point, this variable
                ...
                // Set the data value for this DateTime point to missing,
                // if the number of missing values is greater than the max allowed or if no non-missing values were found.
                // Otherwise compute the new value either as mean or accumulation.
                okToCompute = true;
                if ( dataCount == 0 ) {
            ...
            // If the old ts is >= Day (not stamped at the end), need to advance one old interval,
            // but using the previous old  value to compute the values up to but not including the
            // new data point that coincides with the current old data point.
            // So run the first oldTSi.next() before the loop.
            ...
            for (; oldTSi.next() != null;) {
                ...
                // Check if the old data value is missing and update it according to the missingValueFlag.
                // TODO SAM 2007-03-01 Evaluate use.
                // qualityFlag = "S";
                missingFlag = false;
                if (oldTS.isDataMissing(value)) {
                    ...
                    // Set the missing flag to false if data was left missing.
                    if (oldTS.isDataMissing(value)) {
                        missingFlag = true;
                    }
                }
                ...
                // TODO [LT] 2005-03-01 - This block of code could run only twice,
                // assuming that the data count for the first iteration might be less then the number of
                // actual number of new interval in the old interval.
                // But since this code in general, it apply also to months,
                // when the number of new interval in the old interval will change from month to month.
                // Consider add control flags to improve the performance
                // when running conversions from time series with intervals shorter than month.
                if ( (newTimeScale == TimeScaleType.ACCM) ||
    ...
    public DayTS OLDchangeToDayTS(IrregularTS oldts, int newmult ) {
        ...
        for (; date_new.lessThanOrEqualTo(end); date_new.addDay(newmult), previous_date.addDay(newmult)) {
            ...
            // Now loop through the irregular time series data and transfer to the new date.
            // If the date is past what we are looking for, leave it and use in the next time step.
            ...
            for (; i < iend; i++) {
                ...
                // If the irregular data date is greater than the current loop date,
                // then accumulate and break (so the day interval updates). Also make sure to
                // consider the last observation (move that last check outside the loop to increase performance).
                ...
                if (t.greaterThan(date_new) || date_new.equals(end)) {
                    // Need to break and increment the outside date counter. First do the computations and transfer the numbers.
                    if (count > 0) {
    ...
    public MonthTS newEndOfMonthTSFromDayTS(DayTS oldts, int newmult, Integer bracket, boolean createData ) {
        ...
        // Else get the interval count.
        // This should be a generic procedure that can be applied to other conversions also.
        ...
        for (; t.lessThanOrEqualTo(end); t.addInterval(TimeInterval.MONTH, newmult)) {
            if (nearness > 0) {
                ...
                for (int isearch = 0; isearch < nearness; isearch++) {
                    // Go backward in the current month and forward in the next month until we find a value.
                	// If a value is not found, don't set (leave missing).
    ...
    public MonthTS OLD_changeToMonthTS(IrregularTS oldts, int newmult, PropList proplist) {
        ...
        for (int i = 0; i < iend; i++) {
            ...
            if (i == 0) {
            	...
            	// Not first point checked.
                // Get the previous points data. We can only treat as the same hour if the same day, month, year!
                prev_month = tprev.getMonth();
                prev_year = tprev.getYear();
                if ((month == prev_month) && (year == prev_year)) {
                    // Same hour, so add to the sum if not missing.
                    if (!oldts.isDataMissing(value)) {
                        sum += value;
                        ++count;
                    }
                	...
                	// A different month. Process the previous day and then reset.
                    if (count > 0) {
                        // Compute the average during the day.
                        // Don't worry about the NWS way of doing it where we average one value
                        // from the bounding days since this is irregular data.
                        ave = sum / (double) count;
                        // Now set the value in the new time series. It should be OK to use the.
...
private int getIntervalRelation(TS TS1, TS TS2) {
    ...
    // Using the toSecondsApproximate() because month and year do not allow for the precise computation of toSeconds().
    // Do not use this value to step through the interval, use only for interval comparisons.
    ...
    // Then use these intervals (in seconds) to figure out if one interval is multiple of the other.
    // If so return either 1 or -1, otherwise 0.
