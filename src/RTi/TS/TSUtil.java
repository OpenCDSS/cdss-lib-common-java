...
{	if ( ts == null ) {
	...
	if ( ts.getDataIntervalBase() == TimeInterval.IRREGULAR ) {
		Message.printWarning ( 2, "TSUtil.getTracesFromTS",
		"Can't handle irregular TS." );
		return null;
	}
	int interval_base = ts.getDataIntervalBase();
	int interval_mult = ts.getDataIntervalMult();
	...
	if ( reference_date == null ) {
		...
		use_reference_date = newPrecisionDateTime ( ts, null );
		// Now reset to Jan 1...
		...
		use_reference_date = newPrecisionDateTime ( ts, reference_date);
	}
	//if ( Message.isDebugOn ) {
		Message.printStatus ( 1, "TSUtil.getTracesFromTS",
		"Reference date is " + use_reference_date.toString() );
	//}

	// Because of complexities with the shift, only allow start of year
	// reference dates...

	if (	(use_reference_date.getDay() != 1) ||
		(use_reference_date.getMonth() != 1) ) {
		String message = "To create traces, can only use a reference " +
			"date that is Jan 1 of a year.";
	...
	while ( true ) {
		...
		if ( (newts != null) && do_shift ) {
			Message.printStatus ( 1, "TSUtil.getTracesFromTS",
			"Calling shift with " + newts.getDate1() + " and " +
			use_reference_date );
			shifted_ts = shift ( newts,
				newts.getDate1(), use_reference_date );
			// Replace in the list...
		...
		newts = newTimeSeries ( ts.getIdentifierString(), true);
		newts.copyHeader ( ts );
		newts.setDescription ( date1.getYear() +
		" trace: " + newts.getDescription() );
		newts.addToGenesis ( "Split trace out of time series for " +
		...
		//if ( Message.isDebugOn ) {
			...
			"Created new trace for year " + date1.getYear() +
			" allocated for " + date1.toString() + " to " +
			date2.toString() );
		//}
