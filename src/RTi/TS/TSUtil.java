...
Some of these routines are candidates for inclusion in the TS class,
but are included here first because changes in the TS C++ class require extensive recompiles which slow development.
Putting the code here is almost as efficient.  Most of these classes accept a TS or List of TS as an argument.
@see TS
@see DateTime
@see TSLimits
*/
...
/**
Use the available period.  For example, when adding time series,
does the resulting time series have the maximum (combined period),
...
/**
When used as a property, indicates that a time series transfer or analysis should occur strictly by date/time.
...
*/
	...
	// then a temporary array is needed to track missing.
		...
		if ( tsptIntervalBase == TimeInterval.IRREGULAR ) {
			...
			for ( int i = 0; i < nalltsdata; i++ ) {
				...
				if ( !ts.isDataMissing(data_value) ) {
					...
					data_value = (add + data_value*mult)*factor[i];
				}
...
public static void addConstant(	TS ts, DateTime start_date, DateTime end_date, double add_value ) {
	String  routine = TSUtil.class.getSimpleName() + ".addConstant";
...
The overloaded version is called with missing_flag=SET_MISSING_IF_ANY_MISSING.
@param ts Time series to add to.
@param start_date Date to start adding.
@param end_date Date to stop adding.
...
@param adjustMethod If "Average",
then values on each side of the extreme are added until the average does not exceed the extreme value.
Then each non-missing value is set to the average.
If "WeightedAverage" (currently not implemented),
a similar approach is used except that the adjustment is distributed according to the original data values,
with values that exceed the extreme being set to the extreme.
@param extremeToAdjust AdjustMinimum or AdjustMaximum, indicating whether low or high extremes are being adjusted.
@param extremeValue The extreme value that when exceeded will be adjusted.
For example, specify zero and extremeToAdjust="AdjustMinimum" to adjust negative values.
@param maxIntervals The maximum number of intervals on each side of the extreme
that can be used to computed an adjusted value.
If the value is exceeded without reaching a satisfactory adjustment, do not adjust.
Specify zero to allow any number of intervals.
...
throws Exception {
	String routine = TSUtil.class.getSimpleName() + ".adjustExtremes";
	double oldvalue;
...
/**
Determine whether the intervals for the time series are the same
(the base and multiplier for the interval must agree).
...
The data type, units, etc., will be taken from the first time series in the list and should be reset if necessary after the call.
It is assumed that the time series align in date/time
(i.e., this method cannot be used to shift time series in time before averaging).
@param tslist Time series to get data from, in order to calculate the average.
@param start_date Date to start data analysis (relative to the new average time series).
If null, the earliest date/time is taken from the time series list.
@param end_date Date to stop the data transfer (relative to the new average time series).
If null, the latest date/time is taken from the time series list.
...
<td><b>Indicates how data should be transferred from one time series to another.
Using "ByDateTime" will cause the dates in both time series to match,
which may be an issue if one has leap year data and the other does not.
Using "Sequentially" will result in data from the independentTS to be copied sequentially,
regardless of date/time, which will preserve the continuity of the data but perhaps not the date/time.</b>
<td>ByDateTime</td>
</tr>
</table>
...
throws Exception {
	String  message, routine = TSUtil.class.getSimpleName() + ".average";
	...
	if ( !transfer_bydate ) {
		// Don't specify the end date because without doing some computations, don't know for sure.
		// This should get set to the end date of new TS, which is OK.
...
/**
Blend one time series into another.  Only regular time series can be blended.
The resulting time series will have the combined period of both time series.
Only data after the last date in the original time series will be blended.
...
throws Exception {
	...
	// The output period is at least that of "ts" and if the end of tsb is > ts, then the end date is that of tsb.
	...
	if ( tsb.getDate1().greaterThan(ts_date2_orig) ) {
	    ...
	    // Process a partial tsb time series after the end of the first time series (end of first + one interval).
...
/**
Determine the data size for a time series for a period.
If an IrregularTS data size is needed, call the non-static IrregularTS.calculateDataSize() method.
@return The number of data points for a time series of the given data
interval for the specified period.
This is a utility function to call the time series base class calculateDataSize methods.
An instance of the time series is not required (note that this will not work for IrregularTS,
where the spacing of data is unknown unless a time series is supplied).
...
public static int calculateDataSize ( DateTime start_date, DateTime end_date, int interval_base, int interval_mult ) {
	String routine = TSUtil.class.getSimpleName() + ".calculateDataSize";
...
/**
@return The number of data points for a time series of the given data interval for the specified period.
This is a utility function to call the time series base class calculateDataSize methods.
An instance of the time series is required (use this for IrregularTS, especially).
If the start and end dates are not specific, the time series limits will be used.
...
/**
Convert the units of the specified time series to those requested.
Use the units abbreviations stored in the DataUnits class data.
...
public static void convertUnits ( TS ts, String req_units ) throws TSException {
	double mult, add;
	String rtn = TSUtil.class.getSimpleName() + ".convertUnits";
	int	dl = 20;
	...
	if ( (req_units == null) || (req_units.length() == 0) ) {
		// Probably no need to even notify the user.
		// Some software may call this code even when units are set to a default.
		if ( Message.isDebugOn ) {
			Message.printDebug ( dl, rtn, "No conversion necessary since units are null or blank." );
		}
		return;
	}
	...
	if ( req_units.equalsIgnoreCase(units) ) {
		if ( Message.isDebugOn ) {
			Message.printDebug ( dl, rtn, "No conversion necessary since units \"" + units +
...
/**
Copy one time series header to another.  This simply calls the TS.copyHeader() method.
...
/**
Count the number of data values that are isolated (have missing values on both sides).
This is useful, for example, for determining whether graphs should be drawn with symbols to show the data.
...
throws TSException {
    String routine = TSUtil.class.getSimpleName() + ".createTSDateTimeList";
	List<DateTime> dateTimeList = new ArrayList<>();
    // More than one irregular time series.
	// They at least have to have the same date/time precision for the period.
    ...
    if ( (analysisStart != null) || (analysisEnd != null) ) {
    	// Period has been specified so check the period in analysis.
    	doCheckPeriod = true;
    }
    ...
    while ( true ) {
        ...
        if ( doCheckPeriod ) {
        	...
        	if ( doAdd && (analysisEnd != null) && dtCached.greaterThan(analysisEnd) ) {
        		// Date/time is OK for analysisStart (above) but date/time is after the analysis end.
        		doAdd = false;
        	}
        	if ( doAdd ) {
...
/**
Create a monthly summary report for the time series.
This format is suitable for data intervals less than monthly.
Data are first accumulated to daily values and are then accumulated to monthly for the final output.
This report should not be confused with the MonthTS.formatOutput() method, which just displays raw data values.
@param ts Time series to process.
...
throws Exception {
	// Pull much of the code from the MonthTS.formatOutput() method.
	// This method creates a similar report but does not track data as specifically as is needed here.

	String message, routine = TSUtil.class.getSimpleName() + ".createMonthSummary";
...
"---- --------- --------- --------- --------- --------- --------- --------- --------- --------- --------- --------- --------- ---------" );
	...
	if ( data_interval_base == TimeInterval.DAY ) {
		...
		/* TODO SAM 2004-09-07 Why do anything?  Why take such special care above?
...
*/
		...
		if ( data_interval_base == TimeInterval.DAY ) {
			...
			// First interval in next day, which is still considered part of the previous day because intervals are dated
		...
		// Add the daily value to the month.  Below will check to see if it is time to process the month.
		...
		// The number of values needed in the month is the days since the data are always accumulated to days.
		...
		if ( column == 11 ) {
			...
			// We have been adding to the total, so divide by the number of non-missing for the year if averaging.
...
missing data value if the data are missing.
The header information for the old time series is copied.
The resulting time series can be used for graphing.
...
throws TSException {
	String message, routine = TSUtil.class.getSimpleName() + ".createPeriodOfRecordTS";
	TS newts = null;
...
/**
Disaggregate a time series by converting from a longer interval to a shorter interval.
Currently this is enabled only to convert a 1day (NOT 24Hour) time series to 6hour.
The day's values are transferred to the 6hour time series starting for interval-ending 06, 12, 18, and 00 hours.
@return a new time series that is the disaggregated result.
@param ts Time series to disaggregate.
@param method Method to disaggregate.  If "Ormsbee", use the procedure given by TVA.
If "Transfer", do a simple transfer of values (currently not implemented).
@param new_datatype If "" or "*", transfer the datatype from the original time series.
Otherwise, use the new data type in the disaggregated time series (including the identifier).
...
throws Exception {
	String routine = TSUtil.class.getSimpleName() + ".disaggregate";
	int interval_base = ts.getDataIntervalBase();
	int interval_mult = ts.getDataIntervalMult();
	if ( ts.getDataIntervalBase() == TimeInterval.IRREGULAR ) {
	...
	if ( method.equalsIgnoreCase("Ormsbee") ) {
		...
		// Create a 6-hour time series using as much header information from the original time series as possible.
		...
		// Set the dates.  Going from daily data will result in a start on hour 6
		// (interval-ending value) to hour 0 of the day after the last).
		...
		for ( ; date.lessThanOrEqualTo ( end ); ++nprocessed, date.addInterval(interval_base, interval_mult) ) {
			// Get the initial values.  If necessary, offset the date used in the iterator to get previous/next values,
			// taking care to reset to the "current" value so that it can be incremented correctly at the end of the loop.
			if ( nprocessed == 0 ) {
				// First day value.
			...
			// If all of the input values are zero, set the result to zero (any precision problems with this?).
		...
		if ( (interval_base == TimeInterval.YEAR) && (interval_mult == 1) &&
			...
			// Create a 1Month time series using as much header information from the original time series as possible.
			...
			// Create a 1Day time series using as much header information from the original time series as possible.
			...
			// Create an hourly time series using as much header information from the original time series as possible.
			...
			for ( ; newdate.lessThanOrEqualTo ( newdate2 ); newdate.addInterval(req_interval_base, req_interval_mult) ) {
				if ( req_interval_mult == 24 ) {
					// New hour zero value is the previous day's value.
					// The date was initially set and can be incremented in step with the newdate increment
					// of 24hours since they are equivalent.
					new_value = ts.getDataValue ( date );
					newts.setDataValue (newdate, new_value);
					date.addDay ( 1 );
				}
				    ...
				    if ( newdate.getHour() == req_interval_mult ) {
						// The zero hour will use the previous day's value but others need to use the current day's value.
						date.addDay ( 1 );
						new_value = ts.getDataValue ( date );
					}
					newts.setDataValue (newdate, new_value);
			...
			// Create an minute time series using as much header information from the original time series as possible.
			...
			for ( ; newdate.lessThanOrEqualTo ( newdate2 ); newdate.addInterval(req_interval_base, req_interval_mult) ) {
				if ( req_interval_mult == 60 ) {
					// New minute zero value is the previous hour's value.
					// The date was initially set and can be incremented in step with
					// the newdate increment of 60min since they are equivalent.
					new_value = ts.getDataValue ( date );
					newts.setDataValue (newdate, new_value);
					date.addHour ( 1 );
				}
				    ...
				    if ( newdate.getMinute() == req_interval_mult ) {
						// The zero minute will use the previous hour's value but others need to use the current day's value.
						date.addHour ( 1 );
						new_value = ts.getDataValue ( date );
					}
					newts.setDataValue (newdate, new_value);
...
/**
Divide an entire time series by another time series.
Only regular time series can be multiplied.
If either value is missing, then the result is set to missing.
If the denominator is zero, the result is set to missing.
...
/**
Divide3 a time series by another time series for the indicated period.
Only regular time series can be divided.  If either value is missing, then the result is set to missing.
If the denominator is zero, the result is set to missing.  The units are set to UNITS/UNITS.
...
/**
Enforce limits on the entire time series.
Any time series values greater than the maximum allowed values will be set to the maximum.
@return The total number of time steps changed.
@param ts Time series to check.
@param dates list of dates corresponding to the limiting values.
The date/limit pairs are in effect from the date until the next date encountered.
...
throws TSException, Exception {
	String routine = TSUtil.class.getSimpleName() + ".enforceLimits";
...
It is advisable to always pass in an initial condition date
(e.g. a date and value that are sure to occur before the period of the analysis).
Otherwise, if the first date is after the initial date in the period of record,
the limits will have no effect until that date is reached.
Also, this routine does not handle units or month total conversions
(e.g., it will not take a limits value in ACFT and adjust according to the number of days in each month).
...
incoming limits values by the number of days in the month.
This can be used for example to specify an incoming base limit
(e.g., a flow value converted to volume) that is dependent on an additional conversion.
</td>
...
restriction will be flagged with the given flag value.
A single character is recommended.  The flag will be appended to existing flags.
</td>
...
throws TSException, Exception {
	...
	List<DateTime> limit_dates = new ArrayList<>(num_limit_dates);
...
throws TSException {
	String routine = TSUtil.class.getSimpleName() + ".fillCarryForward";
...
<td><b>FillFlag</b></td>
<td>
A string to use as the data flag when a value is filled.
Typically a one-character string is specified but longer flags can be specified.
</td>
...
/**
Fill missing data in the time series with monthly values.
For example, values[0] is used for any date in January.
This can be used, for example, to fill an entire period with a repetitive monthly pattern.
...
throws Exception {
	...
	TSData tsdata = new TSData(); // Used for irregular data and setting the flag in regular data.
	if ( interval_base == TimeInterval.IRREGULAR ) {
		// Get the data and loop through the list.
		IrregularTS irrts = (IrregularTS)ts;
		List<TSData> alltsdata = irrts.getData();
...
throws TSException {
	String routine = TSUtil.class.getSimpleName() + ".fillDayTSFrom2MonthTSAnd1DayTS";
...
throws TSException {
	String routine = TSUtil.class.getSimpleName() + ".fillFromTS";
...
/**
Fill missing data in the time series using linear interpolation between non-missing values.
There is no limit on how many points can be filled between known points.
@param ts Time series to fill.
@param start_date Date to start assignment.
@param end_date Date to stop assignment.
@exception TSException if there is an error filling the time series.
...
throws Exception {
	String message, routine = TSUtil.class.getSimpleName() + ".fillInterpolate";
	...
	if ( interval_base == TimeInterval.IRREGULAR ) {
		...
		for ( ; date.lessThanOrEqualTo( end ); date.addInterval(interval_base, interval_mult) ) {
			...
			if ( ts.isDataMissing(oldvalue) ) {
				...
				//    In this case additional delta values need to be added to jump over the values before the start period.
				//      num_before_intervals > 1
				...
				// It is possible that non-missing is before the fill start so allow searching to the start of the time series.
...
/**
Fill the time series missing data with monthly values based on the averages for the given pattern.
For example, if Dec, 1995 is missing, and Dec 1995 is a "wet" year according to the pattern,
fill Dec, 1995 with the average of all wet year Decembers.
@param ts Time series to fill.
...
throws Exception {
	String routine = TSUtil.class.getSimpleName() + ".fillPattern";
...
@param ts Time series to fill.
@param pattern_ts Pattern time series.
@param pattern_stats Time series pattern statistics returned from TSUtil.getPatternStats.
In general, this is only used by complicated analysis code.
*/
...
<td><b>FillFlag</b></td>
<td>
Specify a 1-character value to tag filled data values.
If "Auto", the first character of the pattern indicator will be used (e.g., "W" for "WET", "A" for "AVG", and "D" for "DRY").
</td>
No flag is assigned.
</td>
</tr>
...
where the ratio is recalculated each time that values are present in the independent and fill time series.
Data processing can occur forward or backward in time.  The ratio is ts/independent for known points.
For missing values in ts, the value is estimated as ts_value = independent_value*(ratio).
Independent values of zero are ignored when computing the ratio (fillConstant() can be used later to fill with zero if appropriate.
@param ts Time series to fill.
...
@param initial_value Initial value of the fill time series, to compute the ratio on the end of the period.
See the overloaded method for possible values.
...
/**
Fill missing data by prorating known value from an independent time series.
The ratio is ts/independent for known points.
For missing values in ts, the value is estimated as ts_value = independent_value*(ratio).
@param ts Time series to fill.
@param independent_ts Independent time series to find non-missing values to compute the ratio.
@param start_date Date to start assignment.
...
<td>
The end of the analysis period as a date/time string, used when "FactorMethod" is "AnalyzeAverage".
</td>
<td>
...
<td>
The start of the analysis period as a date/time string, used when "FactorMethod" is "AnalyzeAverage".
</td>
<td>
Analyze the full period.
</td>
...
<li>	If "NearestPoint", then the nearest data values from the two time
	...
	The factor is then used until a new value can be calculated.
	The "Direction" parameter will control which points are processed first.
	</li>
<li>	If "AnalyzeAverage" is specified, then the "AnalysisStart" and
	...
	an average factor value for overlapping time series points.
	The constant average factor is then used for scaling the independent time
...
<td>
Direction to fill data, "Backward" or "Forward".  If specified,
the "InitialValue" parameter will be used to substitute for missing data in the
filled time series to compute the initial factor.
This property is only used when FactorMethod=NearestPoint
</td>
<td>"Forward".
</td>
</tr>
...
<td><b>Divisor</b></td>
<td>
Indicate which time series should be used as the divisor (bottom) when computing the ratio,
used with FactorMethod=AnalyzeAverage, either "TS" or "IndependentTS".
</td>
<td>IndependentTS.
</td>
</tr>
...
of the period when observations may not be available.
This property is only used when FactorMethod=NearestPoint.  Possible values are:
<ol>
<li>	NearestBackward - search backward in time from the first value being processed to find a value.</li>
<li>	NearestForeward - search forward in time from the first value being processed to find a value.</li>
<li>	A number - use the specified number as the initial value of the time series,
		when an independent time series value is found and the time series value is missing.
</ol>
</td>
...
throws Exception {
	String  routine = TSUtil.class.getSimpleName() + ".fillProrate";
	String	message;
	...
	boolean DivisorIndependentTS_boolean = true; // Divisor is IndependentTS.
	prop = props.getValue ( "Divisor" );
	if ( (prop != null) && prop.equalsIgnoreCase("TS") ) {
		DivisorIndependentTS_boolean = false;	// Divisor is TS.
	}
    	...
    	if ( Forward_boolean ) {
    		...
    		for ( date = new DateTime(FillStart_DateTime);
    			...
    			if ( ts.isDataMissing ( data_value ) ) {
    			    ...
    			    // Have a data value.
    				// See if the independent time series also has a value that can be used to recalculate the ratio.
    				independent_data_value = independent_ts.getDataValue ( date );
    				if ( !independent_ts.isDataMissing(
    					independent_data_value) && (independent_data_value != 0.0) ) {
...
@return An instance of TSRegression containing the regression information (references to inputs and statistical results).
@param tsToFill Time series to fill.
@param tsIndependent Independent time series.
@param tsRegression A previously computed TSRegression object.
If specified as non-null it will be used rather than computing the regression information from other parameters.
This may be useful for performance reasons and memory management.
...
@param analysisMonths an array of months (values 1 to 12) that indicate the months to analyze.
If one equation is used, then only data for the specified months will be used.
@param transformation the transformation to apply to data.
@param leZeroLogValue if the log transform is used this is the value used to replace values <= zero
(if null use the default given by TSRegression.getDefaultLEZeroLogValue() and NaN will cause the values to be discarded).
@param minimumDataCount the minimum number of overlapping values in a sample to allow filling.
If null, don't have a minimum (other than constraints calculating the statistics).
@param minimumR the minimum correlation coefficient to have a relationship and allow filling.
If null, don't have a minimum.
@param confidenceInterval the confidence interval % to impose - relationships that do not pass the
corresponding level T-test (not a strong relationship) are not used.
If null, the confidence interval is not examined.
...
@param fillStart Date/time to start filling
(this is used for the analysis period for MOVE1 and OLS - MOVE2 uses TSRegression properties for analysis periods).
@param fillEnd Date/time to end filling
(this is used for the analysis period for MOVE1 and OLS - MOVE2 uses TSRegression properties for analysis periods).
...
@param doFill if true, then fill the depending time series;
if false, compute the relationships and statistics, but do not actually fill the time series
...
throws TSException, Exception {
	String routine = TSUtil.class.getSimpleName() + ".fillRegress";
	String message;
...
/**
Fill the missing data within the time series with values based on monthly equations.  Twelve relationships are determined.
@param tsToFill Time series to fill.
@param tsIndependent Independent time series.
@param tsRegression if null, then regression relationships will be determined using the specified parameters.
If not null, an existing regression relationship will be used.
This allows a previously calculated A, B to be used rather than recalculating using data that may have been filled in some way.
@param analysisMethod the regression analysis method to use
@param intercept can be specified as zero for OLS regression to force the relationship through zero, not used with other methods
@param analysisMonths if using one relationship, this specifies that only the specified months (array of values 1-12)
should be considered to develop the relationship, and only those months will be filled.
If monthly relationships, only the specific months will be analyzed and filled.
...
private static TSRegression fillRegressMonthly ( TS tsToFill, TS tsIndependent, TSRegression tsRegression,
	...
	String fillFlag, String descriptionString, boolean doFill )
throws TSException, Exception {
	String routine = TSUtil.class.getSimpleName() + ".fillRegressMonthly";
	String message;
	...
	for ( DateTime date = new DateTime ( start ); date.lessThanOrEqualTo( end );
		...
		try {
			...
			// Error filling interval but just continue.
			// The error is most likely because the month did not have regression relationships.
			;
		}
	}
	...
	if ( fillCount > 0 ) {
    	...
    	// The following comes back as multiple strings but to handle genesis information nicely, break into separate strings.
...
@param tsToFill Time series to fill.
@param tsIndependent Independent time series.
@param leZeroLogValue if the log tranform is used this is the value used to replace values <= zero
(if null use the default given by TSRegression.getDefaultLEZeroLogValue() and NaN will cause the values to be discarded).
@param minimumDataCount the minimum number of overlapping values in a sample to allow filling.
If null, don't have a minimum (other than constraints calculating the statistics).
@param minimumR the minimum correlation coefficient to have a relationship and allow filling.  If null, don't have a minimum.
@param confidenceInterval the confidence interval % to impose - relationships that do not pass the
corresponding level T-test (not a strong relationship) are not used.
If null, the confidence interval is not examined.
@param fill_period_start Date/time to start filling
(this is used for the analysis period for MOVE1 and OLS - MOVE2 uses TSRegression properties for analysis periods).
...
@param descriptionString will be appended to the description
(if not set, "fill regress using TSID" or "fill log regress using TSID" will be used).
@param doFill if true, then fill the depending time series;
if false, compute the relationships and statistics, but do not actually fill the time series
...
private static TSRegression fillRegressTotal ( TS tsToFill, TS tsIndependent, TSRegression tsRegression,
    ...
    String fillFlag, String descriptionString, boolean doFill )
throws TSException, Exception {
	String message, routine = TSUtil.class.getSimpleName() + ".fillRegressTotal";
	boolean regressLog = false;
		...
		try {
    		...
    		if ( tsToFill.isDataMissing(tsToFill.getDataValue(date) ) ) {
    			...
    			if ( !tsIndependent.isDataMissing(x)) {
    				...
    				// Count how many values are filled so that genesis, etc. is only appended to when 1+ values are filled.
    				++fillCount;
    			}
    		}
		}
			...
			// Error filling interval but just continue.
			// The error is most likely because the month did not have analysis relationships.
		}
	}
...
@exception RTi.TS.TSException if there is a problem filling data.
*/
public static int fillRepeat (	TS ts, DateTime start_date, DateTime end_date, int direction, int max_intervals, String fillFlag ) {
	String routine = TSUtil.class.getSimpleName() + ".fillRepeat";
	String message;
...
/**
@return The nearest non-missing data value.  The search will not go past the end of the time series.
Return the missing data value if no non-missing value can be found.
If an irregular time series, always return missing (not implemented for irregular time series).
@param ts Time series to search.
@param date Starting date for search.
@param direction If zero or negative, then for each time step search backwards first.  If positive, search forward first.
@param nforward Indicates how many time steps to search forward.  If zero, there is no limitation on how far forward to search.
@param nback Indicates how many time steps to search backward.  If zero, there is no limitation on how far back to search.
*/
...
/**
Returns the nearest TSData point at which non-missing data is found.
If there are no non-missing data points found then null is returned.
...
@param reverse If true then the iteration will go from date2 to date1.
If false then iteration starts at date1 and finishes at date2.
@return
 */
public static TSData findNearestDataPoint(TS inputTS, DateTime date1, DateTime date2, boolean reverse) {
	...
	String routine = TSUtil.class.getSimpleName() + ".findNearestDataPoint";
...
/**
Returns the index of the time series within the list of time series whose location or full id
(depending upon the PropList) matches the key passed in the parameter list.  Matching just the location is the default.
PropList values can be "MatchLocation=true" or "MatchFullID=true"
*/
public static int findTSIndex ( List<TS> tsList, String key, PropList prop_list ) {
	PropList props;
	if ( prop_list == null ) {
...
/**
Format output for a list of time series, resulting in a standard text output.
This routine calls the formatOutput() member functions for each time series
(some of which may not be implemented at this time).  Null time series are skipped.
@return The lsit of strings for the complete output.
@param tslist List of time series to format.
@param proplist List of properties to modify output.
This list will be passed to the lower-level routines and will be modified as necessary
(e.g. if the overall output is only supposed to have one set of header, comments, and notes,
...
@param fp PrintWriter to receive output.  If not null, then output will be written.
If "fp" is null but "fname" is not, then a writer will be opened.
If both are null, just generate the strings.  This should give a enough flexibility.
...
throws TSException {
	String routine = TSUtil.class.getSimpleName() + ".formatOutput(private)";
	...
	// Loop through the time series and generate a summary for each time series and append to the list.
...
(in general this is used only within the TS package and the version of this routine without the flag should be called).
If calling from a TS class refresh() method, the flag should be false.
@see TSLimits
*/
protected static TSLimits getDataLimits ( TS ts, DateTime start0, DateTime end0, boolean refresh_flag ) {
	String routine = TSUtil.class.getSimpleName() + ".getDataLimits";
	...
	// Loop through the dates and get maximum and minimum data values.
	...
	if ( base == TimeInterval.IRREGULAR ) {
	    ...
	    // A regular TS, so easier to iterate.
...
@param req_units Units to use for check.  If not specified, the units from the first non-null time series will be used.
@param refresh_flag Indicates whether the time series should be refreshed first
(in general this is used only within the TS package and the version of this
routine without the flag should be called).  Specifying true will result in slower execution.
@param ignore_units Indicates whether units should be ignored.
This is suitable for getting the overall data limits for graphs where units can be ignored.
@exception Exception If the data limits cannot be found.
@see TSLimits
*/
...
throws Exception {
	String	message = null, routine = TSUtil.class.getSimpleName() + ".getDataLimits(List,dates)";
	...
	// TODO smalers 2005-04-22 Need to set limits to null, etc., if the final values are the initial (not actual) values.
...
/**
This routine is a port of the legacy TSMonthTotals and may be deprecated at some point.
It is used in the State of Colorado StateDMI software.
...
It is used in the State of Colorado StateDMI software.
@return An instance of MonthTotals containing the monthly totals for the time series.
Data values (sums and averages) are initialized with the missing data value for the time series.
@param ts Time series to evaluate.
...
@param date1 Starting date to evaluate for totals.
@param date2 Ending date to evaluate for totals.
@param reference_ts If specified and the CheckReferencTS property is true,
then the reference time series is checked for missing data, etc., to limit the data that are used.
This can be used, for example, when getting monthly averages for diversion and demand time series.
...
<td>
If <code>true</code>, then the reference time series is checked when analyzing the data.
For example, if a diversion time series is used and demand time series is specified as the reference time series,
other properties can be used to throw out diversion values if demand values are zero.
</td>
<td>false</td>
</tr>
...
<td>
If <code>true</code>, ignore values that are less than or equal to zero (treat as missing).
</td>
<td>false</td>
</tr>
...
in the reference time series is less than or equal to zero (treat as missing).
</td>
<td>false</td>
</tr>
</table>
...
throws TSException {
	int i, month;
	String message, routine = TSUtil.class.getSimpleName() + ".getMonthTotals";
...
/**
Return the list of identifier string given for the time series.  The alias will be returned if set.
Otherwise the full time series identifier string will be returned.
@param tsList list of time series to process.
...
*/
public static String getTimeSeriesIdentifier ( TS ts, boolean includeInput ) {
    if ( !ts.getAlias().equals("") ) {
        ...
        // Use the identifier.
        return ts.getIdentifier().toString(includeInput);
    }
}

/**
Return the list of identifier strings given the time series list.  The alias will be returned if set.
Otherwise the full time series identifier string will be returned.
@param tsList list of time series to process.
@param includeInput if true, include the input part of the time series identifier.
If false, only include the 5-part information.
...
Irregular time series are currently not supported and always return missing data.
...
public static double getInterpolatedDataValue ( TS ts, DateTime date, double timeStepFraction ) {
	...
	if ( base == TimeInterval.IRREGULAR ) {
	    ...
	    // A regular TS, so easier to interpolate.
		// Get the date for the last time step.
		DateTime idate = new DateTime(DateTime.DATE_STRICT);
		idate = new DateTime(date);
		idate.addInterval( base, -1*mult );

		// Get the value from the last time step.
...
/**
Determines time series statistics resulting from the application of a pattern to the time series.
This is called by fillPattern.
...
@param props PropList to modify behavior.
Currently, only "IgnoreLessThanOrEqualZero=true" is recognized.
If true, averages will ignore values <= 0.  The default is false.
*/
public static TSPatternStats getPatternStats ( TS ts, StringMonthTS pattern_ts, PropList props ) {
	String	routine = TSUtil.class.getSimpleName() + ".getPatternStats";
...
The maximum full year bounding period for the calendar type is returned,
even if it means extending the period to include missing data.
The dates are always returned as calendar dates
(e.g., if water years are requested, the first date will be adjusted to be October).
...
@exception RTi.TS.TSException If the period cannot be determined from the limits
(e.g., requesting minimum and there is no overlap).
*/
public static TSLimits getPeriodFromLimits ( List<TSLimits> limits, int por_flag )
throws TSException {
	String 	message, routine = TSUtil.class.getSimpleName() + ".getPeriodFromLimits";
	int	listSize;
	int	dl = 10, i = 0;
	DateTime end, start;

	if ( limits == null ) {
		message = "TSLimits list is null.";
		Message.printWarning(2, routine, message );
		throw new TSException ( message );
	}

	listSize = limits.size();
	if( listSize == 0 ) {
		message="Zero TSLimits list size.";
		Message.printWarning(2, routine, message );
		throw new TSException ( message );
	}
	...
	// If the time series do not overlap, then the limits may be reversed.
...
// listSize	L	Size of the list of incoming time series.
//------------------------------------------------------------------------------

/**
Determine the limits for a single time series.
This is a utility routine to overload the version that accepts a list of time series.
...
/**
Determine the limits for a single time series and a list.
This combination is common where one time series is independent and a list of time series are dependent.
This is a utility routine to overload the version that accepts a list of time series.
...
throws TSException {
	if ( (ts == null) || (tslist == null) ) {
        ...
        // Put single TS and other TS into a list.
		List<TS> v = new ArrayList<>(1);
		v.add ( ts );
		int size = v.size();
		for ( int i = 0; i < size; i++ ) {
...
throws TSException {
	...
	if ( (start == null) || (end == null) ) {
		message = "Unable to get period (all null dates) from " + listSize + " time series (" + nullcount + " null time series).";
		Message.printWarning ( 2, routine, message );
		throw new TSException ( message );
	}
...
This routine may require logic at some point to handle special cases.
For example, the incoming arguments may specify a start date but no end date.
If the start date from the time series is later than the specified end date, then what?
@return The limits given a suggested start and end date.
The date limits can extend beyond the end of the time series dates.
If the suggestions are null, the appropriate start/end dates from the time series are used.
New date instances are created to protect against changing the original dates.
@param ts Time series of interest.
...
The trace ID is not used in the search.
...
/**
Determine whether two time series have matching data intervals.
The intervals must exactly match and not just be equivalent (i.e., 1Day != 24Hour).
The interval base and multiplier from identifiers
...
public static boolean intervalsMatch ( TS ts1, TS ts2 ) {
    List<TS> tslist = new ArrayList<>(2);
    tslist.add ( ts1 );
    tslist.add ( ts2 );
...
/**
Determine whether a list of time series have matching data intervals.
The intervals must exactly match and not just be equivalent (i.e., 1Day != 24Hour).
@param tsList List of time series to check
@return true if all the time series in the list have the same interval as the first time series in the list, false if not
*/
...
/**
Determine the maximum time series values.
The receiving time series description and genesis information are updated to reflect the processing.
...
/**
Determine the maximum values in a list of time series.
The receiving time series description and genesis information are updated to reflect the addition.
@return The time series with maximum time series values in each interval.
@param ts Time series to be modified.
...
throws TSException {
	String message, routine = TSUtil.class.getSimpleName() + ".max(TS,List)";
	int	dl = 20;
	TS tspt = null;
	double add = 0.0, mult = 1.0;
	int set_count = 0;
	...
	for ( int i = 0; i < ntslist; i++ ) {
		...
		try {
			...
			// Can't get conversion.  This may not be a fatal error,
			// but don't want to allow different units to be summed so return.
		...
		if ( tspt_interval_base == TimeInterval.IRREGULAR ) {
			...
			for ( ; date.lessThanOrEqualTo( end_date); date.addInterval(interval_base, interval_mult)){
				...
				if ( tspt.isDataMissing ( data_value_to_check)){
					// The value to check is missing so don't do it.
					// If tracking missing, also set in the array.  This will prevent other time series from processing.
					continue;
				}
...
/**
Determine the minimum time series values.
The receiving time series description and genesis information are updated to reflect the processing.
...
/**
Determine the minimum values in a list of time series.
The receiving time series description and genesis information are updated to reflect the processing.
@return The time series with minimum time series values in each interval.
@param ts Time series to be modified.
@param ts_to_check List of time series to check against "ts".
...
throws TSException {
	String	message, routine = TSUtil.class.getSimpleName() + ".min(TS,List)";
	int	dl = 20;
	TS	tspt = null;
	double add = 0.0, mult = 1.0;
	int set_count = 0;
...
/**
Multiply an entire time series by another time series.  Only regular time series can be multiplied.
If either value is missing, then the result is set to missing.
@param ts Time series to multiply.
...
/**
Multiply a time series by another time series for the indicated period.
Only regular time series can be multiplied.  If either value is missing, then the result is set to missing.
@param ts Time series to multiply.
@param tsm Time series to multiply by.
...
Converting from a low precision to high precision DateTime is typically handled by using first of interval data
(month=1, day=1, hour=0, minute=0, second=0).
If a non-null date is not provided,
then the returned date/time should typically be further manipulated to set its data values.
@return new DateTime of requested precision or null if the precision cannot be determined from the time series.
...
/**
Given a time series identifier as a string,
determine the type of time series to be allocated and creates a new instance.
Only the interval base and multiplier are set (the memory allocation must occur elsewhere).
Time series metadata including the identifier are also NOT set.
@param id time series identifier as a string.
@param long_id If true, then the string is a full identifier.
Otherwise, the string is only the interval (e.g., "10min").
...
throws Exception {
	...
	if ( intervalBase == TimeInterval.MINUTE ) {
        ...
        String message = "Cannot create a new time series for \"" + id + "\" (the interval \"" +
            intervalString + "\", TimeInterval.intervalBase=" + intervalBase + " is not recognized as a regular or irregular interval).";
...
/**
Normalize a time series by linearly interpolating each value between the time series' minimum and maximum values
(or zero, depending "minfromdata"), resulting in values in the range specified by "newmin" and "newmax".
The data type is left the same but the units are set to "".
If no data limits can be found (all missing data), the data values are left as is but the other changes are made.
@param ts Time series to normalized (the time series will be modified).
@param minfromdata Indicate true if the minimum data value to use in the normalization
interpolation should be the minimum time series data value.
...
/**
@return the number of intervals in a month.  This is useful when doing averages, etc.
May need to later make this more generic.
...
Only regular time series can be analyzed.  If either value is missing, then the result is set to missing.
If the denominator is zero, the result is set to missing.
The units are set to "", which assumes that the relative difference is determined using consistent units
(although units are not currently checked).
...
@param divisor The time series to divide by.  Although it can be any time series, it should typically be ts1 or ts2.
@exception Exception if there is an error processing.
*/
...
throws Exception {
	String routine = TSUtil.class.getSimpleName() + ".scale";
...
@param end_date Date to stop scaling.
@param month Month to scale (1-12).  If negative, scale every month.
@param ScaleValue Data value to scale time series, as a number, DaysInMonth, or DaysInMonthInverse.
For the latter two, the number of days in the month, as a floating number, will be used as the scale value.
This is useful for converting between a monthly interval and an interval shorter than monthly.
@exception Exception if the scale value is bad or another error occurs.
*/
...
/**
Select time series from the list by matching criteria.
It is envisioned that this method may be overloaded or transitioned to allow selects based on header information or data.
Currently, its main purpose is to match one or more time series identifiers strings with those in the time series list.
@return a List of matching time series (guaranteed to be non-null but may be zero length).
If the requested TSID string has blank input type and name,
then the check is made ignoring the input type and name in the candidate time series.
If the input type and name are present in the requested TSID, then the input type and name are checked.
@param tslist a list of TS to be checked.
@param tsids a list of time series identifier strings.
The comparison is made by creating a TSIdent for each string and then calling
TSIdent.matches(TSIdentstring-from-TS,true,true),
therefore the alias and input name are checked for a match.  Each matching time series is returned in the list.
...
public static void setConstant ( TS ts, double value ) {
	String routine = TSUtil.class.getSimpleName() + ".setConstant";
...
public static void setConstantByMonth ( TS ts, Double values[] ) {
	String routine = TSUtil.class.getSimpleName() + ".setConstantByMonth";
...
throws Exception {
	String routine = TSUtil.class.getSimpleName() + ".setFromTS";
...
throws Exception {
	...
	if ( interval_base == TimeInterval.IRREGULAR ) {
		...
		// Probably can just use the TSIterator between the two dates regardless of the transfer flag.
		Message.printWarning ( 2, routine, message );
		throw new TSException ( message );
	}
	...
	if ( !transfer_bydate ) {
		// Don't specify the end date because without doing some computations, don't know for sure.
		// This should get set to the end date of the dependentTS, which is OK too.
...
/**
Shift the time series in time (this should not be confused with a numerical add or scale).
This results in a recreation of the time series because data in a month may not shift directly to data in another month
...
@param newdate New date to shift to.
The shift is therefore applied by moving the old date to the new date and shifting all other dates accordingly.
@exception RTi.TS.TSException if there is a problem with input
*/
public static TS shift ( TS oldts, DateTime olddate, DateTime newdate )
throws Exception {
	String routine = TSUtil.class.getSimpleName() + ".shift";
	int	dl = 1;
	...
	*/
	...
	// Compute the offset between the dates.
	// Subtract the first date from the second, assuming that generally offset from the past to the future.
	// This should not affect the final results but needs to be understood for debugging, etc.
	...
	// Now shift to the new dates using the offset between the supplied dates.  This is a new function.
	...
	if ( interval_base == TimeInterval.YEAR ) {
		...
		// Need to overload the constructor to the various TS to take the dates and multiplier so it can call allocateDataSpace directly.
		// Until then, do it the old way.  Also,  have to be careful with the copyHeader stuff.
			...
			if ( olddate_i.getMonth() == 3 ) {
				if ( olddate_i.getDay() == 1 ) {
					if ( (newdate_i.getMonth() == 2) && (newdate_i.getDay() == 29) ) {
						// Add the date 1 by one interval to make things line up.  Will have missing in the new data for the 29th.
						newdate_i.addDay ( 1 );
					}
				}
			}
...
/**
Shift the time series in time (this should not be confused with a numerical add or scale).
The original time series is modified and is returned.
The shift is accomplished by taking values from the original time series at
offsets from the current time and summing the weighted values.
An offset of -1 is the previous time step, 0 is the current time, and 1 is the next time step.
...
throws TSException {
	String routine = TSUtil.class.getSimpleName() + ".shiftTimeByInterval";
	...
	// The copy is used to retrieve data but the original time series values are reset.
	...
	// Will use an array of DateTimes.
	// Set the initial offsets and then iterate by one interval with the other loop.
	// This is much faster than the old code that repositioned the date each time.
	...
	for ( DateTime date = new DateTime(date1); date.lessThanOrEqualTo(date2);
	    ...
	    if ( first_time ) {
	        ...
	        // Increment all of the dates by the interval.
	    	// This will track parallel with the original data and its iterated date.
	        for ( i = 0; i < npairs; i++ ) {
...
/**
Subtract one time series from another.
The receiving time series description and genesis information are updated to reflect the subtraction.
The IGNORE_MISSING flag is used for missing data.
...
/**
Subtract a list of time series from another.
The receiving time series description and genesis information are updated to reflect the addition.
The IGNORE_MISSING flag is used for missing data.
@return The subtracted time series.
@param ts Time series to be subtracted from.
...
/**
Return an array containing the data values (including missing values) of the time series for the specified period.
If the start date or end date are outside the period of record for the time series,
use the missing data value from the time series for those values.
...
throws InvalidTimeIntervalException {
	// Call the version that takes the month and pass a zero month indicating that all months should be processed.
...
/**
Return an array containing the data values of the time series for the specified period.
If the start date or end date are outside the period of record for the time series,
use the missing data value from the time series for those values.
If the start date or end date are null, the start and end dates of the time series are used.
This is a utility routine mainly used by other versions of this routine.
...
*/
public static double[] toArray ( TS ts, DateTime start_date, DateTime end_date, int month_index, boolean includeMissing )
throws InvalidTimeIntervalException {
    int [] month_indices = null;
    if ( month_index != 0 ) {
        month_indices = new int[1];
...
/**
Return an array containing the data values of the time series for the specified period, including missing values.
If the start date or end date are outside the period of record for the time series,
use the missing data value from the time series for those values.
If the start date or end date are null, the start and end dates of the time series are used.
This is a utility routine mainly used by other versions of this routine.
...
*/
public static double[] toArray ( TS ts, DateTime start_date, DateTime end_date, int [] month_indices, boolean includeMissing )
throws InvalidTimeIntervalException {
...
*/
public static double[] toArrayByMonth ( TS ts, DateTime start_date, DateTime end_date, int monthIndex, boolean includeMissing )
throws InvalidTimeIntervalException {
    return toArray ( ts, start_date, end_date, monthIndex, includeMissing );
}

/**
Return an array containing the data values of the time series for the specified period and for a date/time of interest.
The date/time should be the same precision as the time series interval.
For example, if a monthly time series is processed and the date/time is YYYY-01,
then all the January values will be extracted.
If a daily time series is processed and the date/time is YYYY-MM-01, then all the January 1 values are extracted.
If the start or end date are outside the period of record for the time series,
use the missing data value from the time series for those values.
If the start date or end date are null, the start and end dates of the time series are used.
Year, month, day, hour, and minute regular interval time series can be processed.
@return The array of data for the time series.  If an error, return null.  A zero size array may be returned.
An array of TSData is returned to allow the dates corresponding to values to be tracked,
for example when computing statistics like maximum where the date/time of the maximum is of interest.
@param ts Time series to convert data to array format.
@param startDate Date corresponding to the first date of the returned array.
@param endDate Date corresponding to the last date of the returned array.
@param dateTimeRequested a date/time matching the precision of the time series interval,
for which values are to be extracted.
For example, for a daily time series, this is the month and day of the year for which values should be extracted
(the other date/time parts are ignored).
@param includeMissing if true, include missing values; if false, do not include missing values
...
public static TSData[] toArrayForDateTime ( TS ts, DateTime startDate, DateTime endDate,
    ...
    if ( intervalMult != 1 ) {
        throw new InvalidTimeIntervalException(
            "Only Year, Month, Day, Hour, and Minute time series with interval multiplier of 1 can be processed.  Trying to process " +
            ts.getIdentifier().getInterval() + "." );
    }*/
    ...
    if ( intervalBase == TimeInterval.IRREGULAR ) {
        ...
        DateTime date = new DateTime ( start );
        count = 0;
        // FIXME SAM 2009-10-20 This is brute force.  It would be faster to add a year but leap years cause problems.
        // Do this for now unless it is a performance problem.
    ...
    if ( count != size ) {
        // The original array is too big and needs to be cut down to the exact size due to missing data being excluded.
    	// Retain the original object references.
...
/**
Return an array containing the data values (do not include missing values) of the time series for the specified period.
If the start date or end date are outside the period of record for the time series,
use the missing data value from the time series for those values.
If the start date or end date are null, the start and end dates of the time series are used.
