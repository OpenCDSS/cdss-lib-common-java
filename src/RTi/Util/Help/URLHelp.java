...
<pre>
	...
	helphome.addElement ( HBParse.getHome() + "/" + "doc/html/manuals/TSTool" );
...
<p>
The format of the index file is as shown below.
If using the URLHelpGUI interface, it is important to list the topics in an appropriate order because they will be
displayed in the order that they are read from the file.
<p>
<b>To work dynamically, the DOCHOME property is now set internally in the software.
In the readIndex() method, when the DOCHOME property is encountered it is reset to the base directory name of the index file.</b>
...
Keys should not contain spaces and should be as specific as possible to cross-reference with GUI components or topics.
...
/**
Indicates whether the index file has been successfully read.
This is used to determine whether help can be displayed.
*/
protected static boolean _isHelpAvailable = false;

/**
Indicates whether for the current index an attempt to read the index has been made.
This is used to optimize the start-up and eliminate retries to read the index from a remote URL.
*/
...
@return The integer position of the key in the index, starting at zero, or -1 if not found.
...
private static int findIndexForKey ( String key ) {
	...
	for ( int i=0; i < size; i++ ) {
		...
		// If the present key element, i, matches the requested key, return the corresponding index.
...
@return The integer position of the topic in the index, starting at zero, or -1 if not found.
...
private static int findIndexForTopic ( String topic ) {
	...
	for ( int i=0; i < size; i++ ) {
		idata = (URLHelpData)_data.get(i);
		// If the present Topic element, i, matches the requested key, return the corresponding index.
...
/**
Initialize the URLHelp instance.  This method should be called before any specific set methods.  The following tasks are performed:
<ol>
<li>	If the browser parameter is not set, the browser executable is set to
	the default browser on the computer (PC only; UNIX not enabled).
	This requires that the shellcom executable be available.
	If the browser parameter is set, it is used as is.</li>
<li>	The index file is located using the combination of index_paths and index_file.
</ol>
@param browser Path to browser to use.  If null, use the default browser from the system.
@param index_paths Leading path URLs to prepend to the index_file to search for an existing index file.
If not specified, use the index_file without a leading path.  The paths should not include the directory separator.
@param index_file Index file to append to the index_paths, to locate the index file.
*/
public static synchronized void initialize ( String browser, List<String> index_paths, String index_file ) {
	...
	if ( browser != null ) {
		...
		// Run a process to get the default browser.
		// Rely on the timeout rather than a STOP code so that the shellcon program does not need to print a stop code.
	...
	// Now set the help index URL.  The first one that can be successfully read is assumed to be the correct one.
...
An internal check is done to see if the read should be consistent with a call to initialize().
*/
...
protected static synchronized void readIndex ( boolean dynamic_home ) {
	...
	// Initially, assume that help is not available.  If the read succeeds, set to true.
	...
	// Try to get the input stream to the index.  The index may be a URL or a filename.
	...
	try {
		...
		while ( true ) {
			...
			// For some reason, if the URL does not exist, may still get a connection and the first line will be something like:
			// <HEAD><TITLE>404 Not Found</TITLE></HEAD>
			// Check and see if we find a 404 error and if so treat the entire read as a failure.
			...
			if ( Message.isDebugOn ) {
				for ( int i = 0; i < size; i++ ) {
					Message.printDebug ( dl, routine, "Token[" + i + "] = \"" + strings.get(i) + "\"");
				}
			}
			// If the first token has an = sign or the second token starts with =, then we have a property
			// variable and need to save in the property list.
			if (	((size > 0) && (upline.lastIndexOf("=") >= 0))||
				((size > 1) && line.startsWith("=")) ) {
				if ( dynamic_home && upline.startsWith("DOCHOME") ) {
					// Reset the DOCHOME value to the leading path to the index file (which must be an absolute path).
					// This works with files and URLs that may have forward and back slashes.
			...
			// Now try to add the index entry to the list in memory.
			if ( size > 1 ) {
	...
	// If the index was read correctly then all is fine so help is available.

	if ( Message.isDebugOn ) {
		Message.printDebug ( 2, routine, "Read " + _data.size() + " help topics" );
	}
...
/**
Set the path to the browser to execute for on-line help when running stand-alone.
This can be an executable if the executable is in the path.
...
/**
Set a list of String to be displayed instead of trying to display a help topic.
For example, call this with a general message directing users to the PDF documentation.
...
/**
Set the URL that points to the index file.  This function sets the index file path for key and URL addresses for an application.
...
public static void showHelpForKey ( String key ) {
	if ( __help_override_List != null ) {
		// The help override is in place so use it...  For now just Use a standard warning dialog.
