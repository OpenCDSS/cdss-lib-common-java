...
@param rows the number of the rows in the table when this cell editor
was applied.
...
no references to it in the __rowToModel Vector).
@param modelNum the modelNum for which to see how many rows use it as their
data model
...
@param v a Vector of values (SimpleJComboBox data model) to see if is already 
present in the __models Vector.
...
@return true if the new row should use the data model of the row above 
it.
*/
...
@param event the event from which it should be determined whether to select
the cell or not.
...
care of its own internal bookkeeping.
