...
/**
<p>
This is the class from which all the classes that will be used as TableModels in a JWorksheet should be used.
It implements a few core data members that all those classes should have, including some sorting support.
</p>
<p>
TODO (JTS - 2006-05-25) If I could do this over, I would combine this table model with AbstractRowTableModel,
in order to simplify things.  I don't see a very good reason to require both of these, honestly.
</p>
*/
...
/**
The worksheet that this table model works with.  This is here so that it can be set by derived classes.
This class provides a built-in List<T> data array that allows basic table model interaction, such as interacting with rows.
If a List<T> is not appropriate for the data model, then manage the data in the child class and provide appropriate methods.
TODO (JTS - 2004-11-30) remove?  subclasses may be using this ...
TODO (JTS - 2005-03-30) no, leave in, and have it called automatically by the worksheet whenever a model is set in it.
add a setWorksheet() method.
*/
protected JWorksheet _worksheet;

/**
List of integer arrays denoting the cells whose default editability has been overridden.
The int[] arrays consist of:<br><pre>
...
one row depend on the values of the rows before them
(e.g., Time Series dates or running averages).
When a consecutive read is made, the table model is guaranteed that the previous row that
was operated on was either the current row (the column could be different) or the previous row.
</p>
<p>
The default implementation of this method simply pipes through the call to getValueAt(row, column).
</p>
<p>
...
/**
Return the sort order array.
If the data in the table have been sorted, this array is needed to access to original data in the proper order.
*/
...
/**
Used to notify the table model of a new consecutive row read.
Consecutive reads might be used in places where the values of a row depend on the values of the previous rows
(e.g., Time Series dates, running averages).
In a consecutive read, the table model is guaranteed that the row in the call to getValueAt()
is either the same row that was read from last, or one greater than the row that was read from last time.
*/
public void startNewConsecutiveRead() {
}

/**
Can be called upon changing values in a table model.
Notifies all JWorksheet_TableModelListener listeners of the change.
Will not be called automatically by the table model.
