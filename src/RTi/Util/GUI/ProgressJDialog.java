...
/**
This class is a dialog box that can be used to track the completion of a process.
The dialog runs in a thread so it can be updated while other processes are going on.
<p>
<b>Using ProgressJDialog</b><p>
Instantiate the class with the name of the parent frame (the dialog is opened modally),
the title for the dialog, and the min and max values.<p>
<blockquote><pre>
	ProgressJDialog progress = new ProgressJDialog(this, "Copy Progress", 0, numberFilesToBeCopied);
</pre></blockquote><p>

Every time some counter has been updated,
call setProgressBarValue to set the current value (which should be between min and max,
...
/**
Constructor.
@param parent the JFrame on which this dialog appears.  The dialog is opened modally.
...
The value should be &gr;= to the min value passed in to the constructor and &lt;= to the max value passed in to the constructor.
