...
private List<String> _info_List;		// Contains Strings to display in the _info_TextArea object.
...
<tr>
<td>PageLength</td>
<td>No longer used, paging not necessary in Windows NT, NT200), etc or with Java1.2.
If a Windows 95/98/ME machine is detected, the page length is set to 100 regardless of what the property is.</td>
<td>-</td>
</tr>
...
public void actionPerformed( ActionEvent evt ) {
	...
	if ( o == _close_JButton) {
		...
		o = null;
		PrintJGUI.print ( this, _info_List, null, _print_size );
	}
	else if ( o == _save_JButton ) {
		o = null;
		ExportJGUI.export ( this, _info_List );
	}
...
/**
Add the contents of the formatted list to the JTextArea object starting from
...
private void displayContents() {
	...
	if ( prop_value != null ) {
	...
	else if ( _info_List != null ) {
		StringBuffer contents = new StringBuffer();
		String newLine = System.getProperty ( "line.separator" );
		int from = 0; 
		int to = _info_List.size();
		...
		for ( int i=from; i<to; i++ ) {
			contents.append ( _info_List.get( i ) + newLine );
		}
		if (__textComponent.equals("JTextArea")) {
...
private void setGUI() {
	...
	*/
	...
	// Determine the page length in number of lines.
	// NOTE: This was implemented to manage limitations of displayable memory in the the java.awt.TextArea in Windows 95.
	// If a Windows 95 variant, then if the value page length is not set, set it to 100.
	// If not a Windows 95 variant (e.g., NT), then set the value to a large number.
	propValue =  _prop.getValue( "PageLength" );
	String os_name = System.getProperty ( "os.name" );
	if ( propValue == null ) {
		//_page_length = 100;
		// SAM (2001-06-08) - make this large so that paging is off by default.
		// However, if a Windows 95/98/ME machine, set to 100 because these machines cannot handle large reports.
