...
/**
The data table to display in the worksheet.
*/
...
throws Exception {
	...
	this.props = new PropList("DataTable_JPanel.JWorksheet");
...
throws Exception {
	...
	if ( this.filename == null ) {
		...
		if (StringUtil.endsWithIgnoreCase(this.filename, "dbf")) {
			try {
				DbaseDataTable dbaseTable = new DbaseDataTable(this.filename, false, true);
				this.table = dbaseTable;
				fileReadOK = true;
			}
			catch (Exception e) {
				Message.printWarning(2, routine, "Error reading '" + this.filename + "'");
				Message.printWarning(2, routine, e);
				fileReadOK = false;
			}
		}
		else {
			Message.printStatus(2, routine, "Data table in filename '" + this.filename + "' is not supported for reading.");
			throw new Exception("Data table type in filename '" + this.filename + "' is not supported for reading.");
		}
	}
	...
	try {
		DataTable_TableModel tm = new DataTable_TableModel(this.table);
