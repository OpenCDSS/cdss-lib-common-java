...
/**
This class contains records of data as a DataTable, specifically for Dbase file data
(version IV, although many flags are ignored).
Methods are supplied to read Dbase header and data records.
The simplest way to use the class is to call the constructor.
To allow for optimal performance, a table can be read into memory or can remain open for on-the-fly data reads.
Dbase does not seem to make a distinction between integer and floating point data (both are numeric) types,
other than the precision for an integer can be set to zero.
This class currently only returns double precision values for numeric data.
This may impact code where integer values are cross-referenced to other data (e.g., for identifiers).
If problems arise, an option may be added to treat numeric zero-precision values as integer data types.
See:  http://www.dbase.com/Knowledgebase/INT/db7_file_fmt.htm
<b>Note - the getTableRecord() and getTableRecords() methods have not been implemented for on-the-fly reads.</b>
...
/**
Used when reading fields,
the buffer includes a char[] for each field, sized according to the header.
*/
...
/**
Construct a new data table from the Dbase file.
This version is meant to be used when reading from an existing Dbase file
(currently it is not envisioned that the file would be updated, although this enhancement may be added in the future).
...
throws IOException {
	String routine = getClass().getSimpleName() + ".readData";
	int dl = 50;
	...
	// Get the header size in bytes (this includes the main header,
	// the field headers, and the single trailing header terminator).
	...
	// _header_bytes includes the main header and the field descriptors, 32 bytes to describe each field)...
	...
	List<TableField> tableFields = new Vector<>(nfields);
...
/**
Return the field value for the requested record and field index by reading from the file.
This method will overall be a little slower than reading the file sequentially.
However, for cases where the amount of data is large and/or zooming in occurs,
reading only the needed data will often result in fast performance.
This method reads the open database binary file.
@param record_index zero-based index of record
@param field_index zero_based index of desired field
...
throws Exception {
	...
	if ( _field_type[field_index] == 'C' ) {
	...
	else if ( _field_type[field_index] == 'F' ) {
		// Read string and convert to number (float).
		for ( j = 0; j < _field_size[field_index]; j++ ){
			_field_buffer[field_index][j] = _raf.readLittleEndianChar1();
		}
		try {
		...
		// Read string and convert to number (double).
		for ( j = 0; j < _field_size[field_index]; j++ ){
			_field_buffer[field_index][j] = _raf.readLittleEndianChar1();
		}
		try {
...
@param dbf_file Name of dbase file, with or without extension.
@param table Data table to write.
@param write_record An array with a length matching the number of records in the table.
Each boolean in the array indicates whether the record should be written.  If null, all records are written.
@exception IOException if problem writing to file.
*/
...
throws IOException {
	...
	// 8-9 #bytes in header
	// There are 32 bytes in first part of header and then an additional 32 bytes per FIELD descriptor array.
	// Finally, the record header is closed with an additional terminating byte.
	...
	// The record size is the width of each field plus one byte for the delete field at the front.
	...
	for ( int i = 0; i < nrecords; i++ ) {
		...
		for ( ifield = 0; ifield < nfields; ifield++ ) {
			// Write the field data value using the format specification assigned above.
			// The formatString() method cannot just take an Object so need to cast.
			try {
