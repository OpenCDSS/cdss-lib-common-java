...
/**
The MessageLogJFrame class provides a graphical user interface for viewing,
summarizing, and navigating a log file.  For complete functionality,
...
<p>The message levels are interpreted by MessageLogJFrame,
...
Message.print*(..., String Tag,...) methods.
If no tag is provided, then messages will not include the <tag> information.
If the tag is provided, then the MessageLogJFrame is able to notify MessageLogListener instances
...
Use a tag that indicates the commaand/process count (e.g., "ProcessCommands,1").</li>
...
When a user selects a message in the log file summary and then selects the popup
...
Cannot be null.
@param props the PropList that controls the frame's properties.
...
/**
Processes a log file in order to display its text and summary information in the GUI.
...
throws Exception {
	...
	setSize(getWidth() + 200, getHeight());
		...
		// size is just a little small for the worksheet to display nicely.
