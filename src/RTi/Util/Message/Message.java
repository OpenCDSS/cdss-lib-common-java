...
/**
This class provides useful static functions for printing debug, status, and warning messages (see the print* routines).
The class works both for terminal and GUI based applications.
If a message level is zero, then no messages will be generated.
The higher the number, the more detailed the messages.
If a warning message is printed at level 1 and it is from a GUI application,
a modal dialog will appear that forces the user to acknowledge the warning.
...
As a developer, decide on the appropriate message levels in code,
with level 1 messages being visible to the user.
Libraries should generally never print level 1 warning messages.
Errors should be trapped and migrated back to the main application through exceptions.
Use the print* methods to embed message calls in code.
Because debug messages may occur often and because formatting messages is a performance hit,
debug messages should be wrapped in the following code:
<p>
<pre>
...
If using messages with a GUI,
let the Message class know what the top-level component is (for use with the warning dialog, etc.).
<p>
...
The following causes messages to be passed to a function,
which can in turn process the messages (the DiagnosticsJFrame does this).
In the future, a listener approach may be taken.
<p>
...
/**
The following are used when setting message output locations.
Output to the terminal (UNIX or DOS shell).i
...
/**
Output to a scrolling status area within ProcessManagerDialog or ProcessManagerJDialog.
This may not be needed after 2002-10-16 revisions to the ProcessManager.
...
/**
A list of output names corresponding to:
  TERM_OUTPUT
  LOG_OUTPUT
...
/**
Indicates whether debug is on.  This is public so that it can be quickly accessed.
To increase performance, all debug messages should be wrapped with:
<p>
<pre>
if ( Message.isDebugOn ) {
...
The default to no debugging being done.  Set to true in command-line parsing code if setDebug is called.
*/
public static boolean isDebugOn = false;

/**
The following are flags for printing messages, set by various methods in this class.
The following setting will print the message level in debug and warning messages
(the default is to not show the level).
The Diagnostics GUI will at some point edit all of these settings.
...
/**
Flush the output buffer after each message is written.
The default is to let the system flush the output buffer.
*/
...
The listeners are updated by the MessageLogJFrame every time its processLogFile() method is called and a log file is read and displayed.
...
This can be used by code that needs to display a dialog and otherwise does not have knowledge of the main application JFrame.
...
/**
Open the log file.
Because no log file is specified, the name of the log file will default to the program name and the extension ".log"
(set with IOUtil.setProgramName).
...
/**
Print information about the registered message levels using status messages, using the current message settings.
*/
...
/**
Call printWarning but specify a different _top_level frame from the preset top level frame.
This is useful when you are printing a warning of level 1 and want to specify which window
that WarningDialog should be associated with, without changing the top level window that should be typically used.
*/
...
/**
Print a warning message to the registered output receivers.
@param level Warning level for the message.  The level will be printed with the message if the ShowMessageLevel property is "true".
...
/**
Removes a listener from the Vector of listeners that are set to listen to actions from the MessageLogJFrame.
...
This is NOT called by the setDebugLevel method if the debug level is greater than zero for any output receiver.
...
These properties currently are used mainly to control the warning dialog behavior.
...
The default is to NOT show message levels.  If true, the normal messages will be modified to include the level in [brackets],
as follows (the optional Tag and Routine are shown for illustration):
<pre>
...
If true, the normal messages will be modified to include the tag in <brackets>,
as follows (the optional Tag and Routine are shown for illustration):
<pre>
Warning[Level]<Tag>(Routine)...
</pre>
