...
/**
This class is used to perform the <B>Timable.performTimedAction()</B> function at a regular interval.
The sleep time can be specified in minutes, seconds or milliseconds. The class is used as follows:
...
In the above example, the performTimedAction() method of <B>MyClass</B> is performed every minute.<BR>
<P>
...
@param timable Instance of the Timable interface which has the performTimedAction() methods.
@param sleep_time Time to sleep.
@param uflag Units of the sleep time (MINUTES, SECONDS, or MSECONDS).
@exception IllegalArgumentException Throws this if any of the arguments are not valid.
*/

public 	TimingThread( Timable timable, long sleep_time, int uflag )
	throws IllegalArgumentException {
	if( timable == null ){
		throw( new IllegalArgumentException( "NULL Incoming Timable!" ) );
	}
	_timable = timable;
...
Construct and start the timing thread with the desired settings.
@param timable Instance of the Timable interface which has the performTimedAction() methods.
@param thread A thread that is to be suspended while the performTimedAction() method is fired.
@param sleep_time Time to sleep.
@param uflag Units of the sleep time (MINUTES, SECONDS, or MSECONDS).
@exception IllegalArgumentException Throws this if any of the arguments are not valid.
*/
...
throws IllegalArgumentException {
	...
	if( thread == null ){
		throw( new IllegalArgumentException( "NULL Incoming Thread!" ) );
	}
	_thread = thread;
...
/**
The run function is called automatically from the constructors.
It does the majority of the work of this class: sleeping for desired time,
suspending the secondary thread if desired, and performing the <B>performTimedAction</B> function.
*/
...
private void setSleepTime( long sleep_time, int uflag )
	...
	if( sleep_time == 0 ){
		throw( new IllegalArgumentException( "Zero Sleep Time!" ) );
	}
	...
	if( uflag == MSECONDS ){
		...
		throw( new IllegalArgumentException( "Unrecognized units flag " + uflag + "." ) );
	}
