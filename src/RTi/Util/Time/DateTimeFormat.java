...
/**
<p>
This class aids in the parsing and formatting of DateTimes according to String formats.
A major factor in the design of its interface methods was that
...
<tr>
<td>mm</td>
<td>Represents a 2-digit month.  If the month is in the range 1-9,
it must be padded with a single zero (e.g., "03").</td>
</tr>

<tr>
<td>m</td>
<td>Represents a 1- or 2-digit month.  If the month is greater than 9,
it will be represented by 2 digits.  If the month is in the range 1-9, it will be a single digit.</td>
</tr>

<tr>
<td>dd</td>
<td>Represents a 2-digit day.  If the day is in the range 1-9,
it must be padded with a single zero (e.g., "03").</td>
</tr>

<tr>
<td>d</td>
<td>Represents a 1- or 2-digit day.
If the day is greater than 9, it will be represented by 2 digits.
If the day is in the range 1-9, it will be a single digit.</td>
</tr>

<tr>
<td>hh</td>
<td>Represents a 2-digit hour.
If the hour is in the range 0-9, it must be padded with a single zero (e.g., "03").</td>
</tr>

<tr>
<td>h</td>
<td>Represents a 1- or 2-digit hour.  If the hour is greater than 9, it will be represented by 2 digits.
If the hour is in the range 0-9, it will be a single digit.</td>
</tr>

<tr>
<td>MM</td>
<td>Represents a 2-digit minute.
If the minute is in the range 0-9, it must be padded with a single zero (e.g., "03").</td>
</tr>

<tr>
<td>M</td>
<td>Represents a 1- or 2-digit minute.
If the minute is greater than 9, it will be represented by 2 digits.
If the minute is in the range 0-9, it will be a single digit.</td>
</tr>

<tr>
<td>ss</td>
<td>Represents a 2-digit second.
If the second is in the range 0-9, it must be padded with a single zero (e.g., "03").</td>
</tr>

<tr>
<td>s</td>
<td>Represents a 1- or 2-digit second.
If the second is greater than 9, it will be represented by 2 digits.
If the second is in the range 0-9, it will be a single digit.</td>
</tr>

<tr>
<td>ff</td>
<td>Represents a 2-digit hundredth of second.
If the hundredth of second is in the range 0-9, it must be padded with a single zero (e.g., "03").</td>
</tr>

<tr>
<td>f</td>
<td>Represents a 1-digit hundredth of second.
If the hundredth of second is greater than 9, only its tenth value (the left-most digit) will be shown.
If the hundredth of second is in the range 0-9, it will be shown as 0.
</td>
</tr>

<tr>
<td>tz</td>
<td>Represents a string time zone.
Currently on a single time zone type (all characters and numbers, no whitespace) is supported.<p>
...
<b>Note:</b> "date/time piece" is a term used throughout this documentation,
and it refers to a discrete part of a Date Time, such as a year, month, day, etc.
...
Used internally as the position of recognized date/time pieces within internal arrays.
These are used instead of TimeInterval.* or DateTime.PRECISION_*
because they are neither intervals nor precisions, and because they are used
as internal array element specifiers and the TimeInterval and DateTime values have different values.
<p>
...
/**
String arrays storing the various format specifiers recognized for date/time pieces.
This is a two-dimensional array because a single date/time piece can
be represented by several different format specifiers.
For example: Years can be "yyyy" or "yy".<p>
...
/**
String arrays storing the various regular expressions to use for recognizing date/time pieces.
This is a two-dimensional array because a single date/time
...
/**
Whether the format in this DateTimeFormat is absolute or not.
An absolute format will specify a DateTime at a single point in time.
For example, "2005", "2005-12", and "2005-12-25" are all absolute points in time.
"05", "2005 16:32", and "12/25 16:32" are relative points in time.
One way of easily determining whether a format is absolute or not is to see whether the
...
/**
A copy of the format String, in character-array format.
This is used internally to track which format specifiers in the format String have been processed and which have not.<p>
For instance, if a DateTimeFormat string is "yyyy-mm",
then when this array is first instantiated it will contain "yyyy-mm".
Once the "yyyy" format specifier has been handled internally, this array will then contain
"....-mm", where "." is actually the character specified by __used.
...
method to represent a character that has already been processed into a regular expression.
This character value corresponds to the ASCII EOT value,
and thus should never appear within a DateTimeFormat string.<p>
...
the __formatCharacters array in its characters so that it is known not to reprocess it.
The __formatCharacters array will then be "4444-mm-dd".
*/
private final char __used = 4;

/**
Integer arrays that mark the location of each format specifier in the format String.
The first two dimensions of this array correspond to the two
dimensions of the __formatSpecifiers and __formatSpecifierRegEx arrays (i.e.,
the first dimension corresponds to the date/time piece (e.g., __YEAR),
and the second dimension corresponds to particular format specifiers [e.g., "yyyy", "yy"]).<p>
...
<b>NOTE:</b> This is a very complicated set of arrays,
but it is only used to accumulate information about the format specifiers during initial set up.
...
/**
An array of the location of all format specifiers found in the DateTimeFormat string, sorted into ascending order.
Thus, traversal through the array will
reveal the indices of each format specifier in the String in left-to-right order.
See also
...
in the DateTimeFormat string, sorted in the same order as <code>__sortedFormatSpecifierLocations</code>.
Thus, traversal through the array will result in the regular expression of each format specifier in the
DateTimeFormat string from left to right.<p>
The location in the format String of the format specifier with the regular
expression at <code>__sortedFormatSpecifierRegExs[X]</code> is available from <code>__sortedFormatSpecifierLocations[X]</code>.
See also <code>int[] __sortedFormatSpecifierLengths</code> and
...
/**
An array containing the literal values that surround all the format specifiers in the format String.
The array is set up such that the element at position X
...
specifier, and the last element contains the literal values that follow the final format specifier.<p>
...
/**
The string that corresponds to the regular expression constructed to parse the given DateTime format.
*/
...
No format is considered a bad format.
If no format specifiers are contained within the format,
it will simply output only literal values when a date is formatted for output,
and will recognize only strings that match the exact value of the format string for input.
...
public DateTimeFormat(DateTimeFormat format) {
	// The following is rather complicated.  Copies are made of all the internal data from the original format.
	// If this proves to be a maintenance hassle, it might be better just to do this, instead:
	/*
		public DateTimeFormat(DateTimeFormat format) {
			this(format.__format);
		}
	*/
	// That'll have more performance overhead, but it might be a better tradeoff given the complexity of the following:
...
/**
Creates the internal data structures necessary to format a DateTime object as a String with a DateTimeFormat string.
*/
private void createFormatter() {
	// __formatSpecifierLiteralSurroundingValues needs to be one element larger than the sorted arrays.
	// It contains the literal characters that precede a format specifier (but that are after the previous format specifier).
	// The final element's values do not precede a /format specifier,
	// but are the final literal characters in the string, hence the +1 size.
	...
	// The first format specifier's preceding characters are an exceptional case,
	// since there is no prior format specifier.
	...
	for (int i = 0; i < __sortedFormatSpecifierLocations.length; i++) {
		// The last format specifier's following characters are an exceptional case,
		// since there is no following format specifier.
...
/**
Creates arrays with the location, type, and length of the DateTimeFormat string format specifiers,
and of the regular expressions needed to parse those format specifiers,
in the order that the format specifiers appear in the DateTimeFormat string, from left to right.<p>
...
private void createSortedArrays() {
	...
	for (int i = 0; i < __NUM_PIECES; i++) {
		...
		for (int j = 0; j < size; j++) {
			for (int k = 0; k < __formatSpecifierLocations[i][j].length; k++){
				locs.add(Integer.valueOf(__formatSpecifierLocations[i][j][k]));
				types.add(Integer.valueOf(i));
				lengths.add(Integer.valueOf(__formatSpecifiers[i][j].length()));
				regExs.add(__formatSpecifierRegEx[i][j]);
			}
		}
	}

	// First, move the values from "locs" into an array, so that the array can be sorted.
	// These values are the starting positions of each format specifier within the DateTimeFormat string,
	// and once formatted will be in left-to-right order moving through the string.

	size = locs.size();
	__sortedFormatSpecifierLocations = new int[size];
	int[] order = new int[size];
	for (int i = 0; i < size; i++) {
		__sortedFormatSpecifierLocations[i] = locs.get(i).intValue();
	}
	...
	// Transfer data from the other lists into their respective arrays,
	// in the same order as the original values were moved from "locs" to "__sortedFormatSpecifierLocations".

	// That is, an element at position X in locs is now in position Y in __sortedFormatSpecifierLocations.
	// Do the same thing, for lists "types", "lengths", and "regExs" with their respective arrays

	for (int i = 0; i < size; i++) {
		__sortedFormatSpecifierTypes[i] = types.get(order[i]).intValue();
		__sortedFormatSpecifierLengths[i] = lengths.get(order[i]).intValue();
		__sortedFormatSpecifierRegExs[i] = regExs.get(order[i]);
	}
}
...
private void createRegularExpression() {
	...
	for (int i = 0; i < length; i++) {
		// First, see if the given position (i) is one at which a format specifier starts.
		// This method returns the index within the sorted arrays of the format specifier that starts at the given position.
		...
		if (marked == -1) {
			...
			// If the character is a backslash, then this is an escaped literal character (such as \m).
			// Characters that are used in format specifier strings must be escaped.
			...
			// Any character that is not a normal number or letter should be escaped,
			// because many of them have secondary meaning in regular expressions.
			if (needsEscaped(ch)) {
				sb.append("\\");
			}
			sb.append("" + ch);
			...
			// Copy the regular expression that corresponds to format specifier at the given position to the buffer.
			sb.append(__sortedFormatSpecifierRegExs[marked]);

			// Move ahead enough characters to move off the current format specifier.
...
/**
When a <code>parse()</code> call fails,
calling this method on the same inputString string will result in an explanation of where the parsing failed.
The explanation will be returned as a String array, each element of which
contains different information about how the parse failed.
This is so that applications can choose which information they wish to display.<p>
<b>NOTE:</b> This is a <b>very</b> expensive method to call, and should not be called in a loop for many parses().
At its worst case, performance-wise it is equivalent to calling parse() a number of times equal to the number of
characters in all the format specifiers in the DateTimeFormat string.
At its best, it is equivalent to calling parse() twice.<p>
...
[1] = INPUT<br>
[2] = a carat pointing to the spot in INPUT where parsing failed, like what the java compiler shows upon failure.<br>
...
public String[] explainParseFailureOld(String inputString) {
	...
	// This essentially tries to validate the string one character at a time until it finds where the validation fails.
	// It can then tell which character in the input string caused the failure.

	// Given a format String "mm-dd", the following is its regular expression for parsing:
	//	(\d\d)-(\d\d)

	// When this method is called,
	// it will try to generate and run all the following regular expressions, until one fails:
	//	(\d).*
	//	(\d\d).*
	//	(\d\d)-.*
	//	(\d\d)-(\d).*
	...
	// As should be obvious, for long DateTimeFormat strings,
	// this could turn out to be a lot of regular expressions.
	// There is almost certainly a better way to do this, but limited investigation
	// revealed no leads on alternate methods, at least given Java 1.4.2's RegEx library.
...
/**
Returns a 5-element array of Strings that explains why a String could not be parsed.
An example of the return array is:<p>
...
public String[] explainParseFailure(String inputString) {
	...
	// time until it finds where the validation fails.
	// It can then tell which character in the input string caused the failure.
	...
	// it will generate and all the following regular expressions and cache them:
	//	(\d).*
	//	(\d\d).*
	//	(\d\d)-.*
	//	(\d\d)-(\d).*
	...
	// After that (and immediately upon the second to Nth time this method is called),
	// each one is run upon the input string in turn in order
	// to find the point at which the parsing failed.

	// As should be obvious, for long DateTimeFormat strings, this could turn out to be a lot of regular expressions.
	// There is almost certainly a better way to do this, but limited investigation
	// revealed no leads on alternate methods, at least given Java 1.4.2's RegEx library.
	...
	if (__failurePatterns == null) {
		...
		// Holds the regular expressions used to create the Patterns in the patterns list.
		...
		for (int i = 0; i < size; i++) {
			__failurePatterns[i] = patterns.get(i);
			__failurePatternStrings[i] = patternStrings.get(i);
		}
	}
	...
	for (int i = 0; i < size; i++) {
		...
		if (dieFast || !matcher.matches()) {
			...
			errors[1] = inputString;
			errors[2] = errorLocation;
			errors[3] = "The original regular expression used to try parsing was: \"" + __regularExpressionString + "\".";
			errors[4] = "The regular expression that caused parsing failure was: \"" + __failurePatternStrings[i]+ "\".";
			__last = i;
			return errors;
		}
		else {
...
/**
Once a Regular Expression Matcher has successfully matched a DateTime string to a regular expression,
move the values that were found with the Matcher into a DateTime object.<p>
...
@param dt the DateTime object to fill with the matched values.
If null, a new DateTime object will be instantiated.
*/
private DateTime fillDateTime(Matcher matcher, DateTime dt) {
	...
	// - tried using a DateTime instantiated here and returned with the values in it.
	//   Each time, the same DateTime would be used rather than creating one every time.
	//   This offers a decent speed increase (~10%),
	//   but the returned value has to be used in a copy constructor in the calling code,
	//   anyway, thus eliminating the performance gain.
	//   It's possible this might be useful in some applications later on, so this should be kept in mind.
	...
	// - turning off STRICT processing, but not turning on FAST (eg, passing in a flag value of 0).
	//   Processing actually slowed down slightly (~2%) due to how the flags are handled internally in the DateTime class.
	// This is a note to future developers who may think there are optimizations that can be done here.
	...
	// Finer-grained units of time have higher precision,
	// so a Day has higher precision than Month or Year, but lower precision than Hour, Minutes, etc.
	int highestPrecision = -1;
	// The integer value of the value matched by the regular expression Matcher.
	int val = -1;
	// The String matched by the regular expression Matcher.
	String matchedString = null;

	// The following is primarily used to pass in the DATE_FAST flag, which results in a slight (10-15%) performance gain.
	...
	for (int i = 0; i < __sortedFormatSpecifierTypes.length; i++) {
		...
		// Matcher.group(0) is equivalent to matcher.group(),
		// which returns the entire string that was matched against (i.e., the date string being parsed).

		matchedString = matcher.group(i + 1);

		switch (__sortedFormatSpecifierTypes[i]) {
			case __YEAR:
				// Special handling is required for 2-digit years.
				// The handling method here is to translate any years less than 30 to mean
				// the 21st century, and any greater than or equal to 30 be in the 20th century.
				// This is consistent with the standard Microsoft Y2K default handling.
			...
			case __SECOND:
				...
				break;
			case __HSECOND:
				// Special handling is required for 1-digit hundredth-of-second values.
				// Because the first value is the tenths of digits,
				// if it is the only part showing it must be multiplied by 10 to have the proper magnitude.
				if (matchedString.length() == 2) {
					val = StringUtil.atoi(matchedString);
				}
				else {
...
/**
Given a relative date/time and an absolute date/time,
this fills the missing values in the relative date/time with the values from the absolute date/time.<p>
For example, if the relative date/time was "14:55" and the absolute date/time was "2006-05-23 12:30:44",
the relative date/time would be filled with the year, month, day and seconds from the absolute date/time,
resulting in a new relative date/time of "2006-05-23 14:55:44".
...
(e.g, __YEAR, __MINUTE).
@param formatSpecifierNum the number of the format specifier being searched for.
This corresponds to the different types of format specifiers available
...
private void findFormatSpecifierOccurrences(int formatSpecifierType, int formatSpecifierNum) {
	...
	while (index != -1 && !done) {
		if (__formatCharacters[index] == __used) {
			// Skip it -- found by a higher-priority format specifier and marked as used.
			// "Higher-priority" means that the format specifier is searched for before others.
			// For example, if a format is "Year: yyyy",
			// then "yyyy" will first be matched by the higher-priority format specifier of "yyyy" and processed.
			// If it were processed by the lower-priority format specifier
			// ("yy"), it would be translated into two two-digit year values, rather than a single four-digit year value.
			start++;
		}
		else if (isEscaped(formatSpecifier, index)) {
			// Skip -- this is an escaped literal character, not one that should be in a format specifier.
			start++;
		}
		else {
			// Found a format specifier.
			// First, mark this format specifier as used so that later format specifier collections will not attempt to reuse it.
			for (int i = index; i < (index + length); i++) {
				__formatCharacters[i] = __used;
			}

			// The above is done so that "yyyy" will only be parsed as a four-digit year, rather than as two two-digit years.

			// If the characters were not marked as __used,
			// they would be matched and re-matched by lower-priority format specifiers.

			// For instance, in this format string: "yyyy-mm" once "yyyy" is matched,
			// this format becomes "xxxx-mm" (though instead of "x" it is an EOT character).

			// If it weren't, then when the next kind of year format specifier was sought,
			// it would find match against the two "yy" strings in the above format.
	...
	// Translate the locs list into an array and put its values into __formatSpecifierLocations.

	__formatSpecifierLocations[formatSpecifierType][formatSpecifierNum] = new int[locs.size()];
	int size = locs.size();
	for (int i = 0; i < size; i++) {
		__formatSpecifierLocations[formatSpecifierType][formatSpecifierNum][i] = locs.get(i).intValue();
	}
}

/**
Formats a DateTime object according to the DateTimeFormat string this DateTimeFormat object was constructed with.
...
public String format(DateTime dt) {
	...
	for (int i = 0; i < __sortedFormatSpecifierLocations.length; i++) {
		...
		// Add the literal characters that precede the current format specifier in the format String.

		sb.append(__formatSpecifierLiteralSurroundingValues[i]);

		// Based on the type if the current format specifier, retrieve the appropriate value from the DateTime.
		...
		if (__sortedFormatSpecifierTypes[i] != __TZ) {
			if (__sortedFormatSpecifierTypes[i] == __YEAR) {
				// Years need to be handled specially.  4-digit years can be output as they are,
				// but otherwise, just take the final two digits from the year and output it.
				if (__sortedFormatSpecifierLengths[i] != 4) {
					// Formats the output as a two-digit year.
					// First, the year has to be modded to get only the tends and ones values from it.
					val = val % 100;
					temp = StringUtil.formatString(val, "%02d");
					sb.append(temp);
				}
				else {
					// Formats the date as a string at least 4 characters long (errors will occur with years greater than 9999)
				...
				// Hundredths-of-seconds must be handled specially.
				// If the formatting string is "ff" then the full value is printed,
				// but otherwise the tens value of the number must be printed and the ones value must be removed.
				temp = "" + StringUtil.formatString(val,"%02d");
...
/**
Given a range of precisions, returns formats that can be displayed as choices for formatting.
The precisions passed to this method filter the choices to be shown.  <p>
No formats containing values less precise than the first precision will be shown,
and no formats containing values more precise than the second precision will be shown.
For instance, with a first precision of Year and a second precision of Day,
the following would be among the choices returned: "yyyy-mm-dd",
whereas with a first parameter of Month and second parameter of day,
it would never be because it contains year formatting information, and that is less precise than Month.
@param maxInterval the highest (least precise) interval for which to list formats.
This value should be less precise or equal to than the minInterval value.
For instance, if this value is DateTime.PRECISION_MONTH,
the other parameter should not be DateTime.PRECISION_YEAR, but could be DateTime.PRECISION_DAY.
@param minInterval the lowest (most precise) interval for which to list formats.
This value should be more precise or equal to than the maxInterval value.
For instance, if this value is DateTime.PRECISION_MONTH,
the other parameter should not be DateTime.PRECISION_MINUTE, but could be DateTime.PRECISION_YEAR.
...
/**
Returns whether the format includes the given date/time piece.
This can be used to determine easily whether a format will display years, for instance, by:<p>
<code><pre>
	DateTimeFormat dtf = ...
	...
...
A date is considered absolute if it contains Year, or Year and Month,
or Year and Month and Day, etc., on through Year to Hundredth-of-second.
If the format refers to a specific point in time, then it is absolute.
If not, it is relative.<p>
For example:<p>
"yyyy-mm" specifies to a specific year and month.
Though it is not any more precise than that, it is still a specific point in time and is absolute.<p>
"yyyy-dd" specifies a specific year and specific day,
but the month is not specified and thus this format could refer to any month within the given year.
It is not absolute.<p>
"mm-dd" specifies a specific month and day, but not a specific year and thus could apply to any year.  It is not absolute.
...
determining if a new instance of a DateTimeFormat is absolute, in its constructor.
See the DateTimeFormat constructor to see how this is called.
@param format the DateTime format String to check.
...
private boolean isEscaped(String formatSpecifier, int index) {
	...
	if (index == 1) {
		// Only have to check back one character (not possible to have another "\" at the -1st position).
		...
		// Look back through the string to make sure that if this format specifier is escaped,
		// that its escape value isn't actually escaped itself.  That is, this:
		// 	\\mm
...
@return the index of the format specifier (within the __sortedFormatSpecifier* arrays)
found at the specified index, or -1 if none occur at the given position.
*/
...
/**
Increments to the next relative date time from the given date time.
None of the fields that are specified in the format for this DateTimeFormat will be changed,
but the others will increment and iterate properly.
The fields that are not specified in the format will iterate from whatever their current values in the DateTime object are.
Starting values for these fields must be specified in the calling code, or they will default to 0s and 1s.<p>
A different format must be used to show the iterated values,
as they are not present in the DateTimeFormat object that is used to iterate.<p>
Relative DateTimeFormats offer the ability to specify a date that is not tied to a specific point in time.
"July 4th" could be 2005-07-04, 1996-07-04, 1723-07-04, etc.
Because relative dates do not specify a single point in time,
they can be used to specify a date that repeats in time.
The date/time pieces that are present in the format (in the above, Month and Day) remain constant,
while the other date/time pieces can vary.
This method offers an easy way to repeat a relative date/time across time.<p>
This method only does a single increment of the relative DateTime,
and so iteration must be handled in the calling code.<p>
For example, this is used in annotation drawing code on a TSGraph to specify a range of dates for an alert level.
An alert level line is specified by two dates in the format: "mm/dd".
The left-most year on the graph is used as the starting year and the dates are iterated (with this method)
through all the years drawn on the graph.<p>
This concept is confusing, and it is best illustrated by examples
The following show the format of the DateTimeFormat,
the original value of the DateTime, and its values for 5 iterations.
All the iterated values are shown with the following format:
yyyy-mm-dd hh:MM<p>
<b>Example 1</b><p>
"mm hh:MM"  (2005-10-29 11:30)<p>
This example can be used to iterate through all the days in October for every year, when the time is 11:30 AM.<p>
- 2005-10-30 11:30<br>
- 2005-10-31 11:30<br>
- 2006-10-01 11:30<br>
- 2006-10-02 11:30<br>
...
so if its original values should be maintained, a copy should be made prior to calling this method.
@param dt the DateTime to iterate.  This DateTime object is changed within the method and returned,
so if its original values should be retained a copy should be made.
...
/**
Parses the given input date and returns a DateTime containing its values.
If the input string could not be parsed, null will be returned.See
...
/**
Parses the given input date and returns a DateTime containing its values.
If the input string could not be parsed, null will be returned.
See <code>explainParseFailure()</code> if further information is necessary as to why a parse failed.
@param inputString the input string containing a date to parse.
@param dt the DateTime object to fill with the values in the given inputString.
...
/**
Sets the flag with which the DateTime constructor will be called when a DateTime is parsed from an input String.
By default, new DateTime objects are created with the default constructor,
which specifies a flag of DateTime.DATE_STRICT.  Passing in a value of <code>DateTime.DATE_FAST</code>
will result in a slight performance gain when parsing DateTimes.
Setting to fast by default is not enabled because setting to fast disables some date checks
(e.g., finding the day of the year and leap-year status), which may not be desirable in all situations.
...
/**
Returns a String representation of this object.
@param verbose if true, much extra information on the internal data members is printed, as an aid to debugging parsing.
@return a String representation of this object.
*/
