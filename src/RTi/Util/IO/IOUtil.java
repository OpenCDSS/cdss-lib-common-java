...
{	String	comment, routine = "IOUtil.printCreatorHeader";
	...
	if ( props != null ) {
		...
		if (	(prop_value != null) &&
			...
			is_xml = true;
			// If XML, do not print multiple dashes together in
			// the comments below.
		}
	}
	...
	ofp.print ( comment + " command:   " + _progname + " " );
	...
	if ( _argv != null ) {
		for ( i = 0; i < _argv.length; i++ ) {
			...
			if ( (column + 1 + len) > maxwidth ) {
				...
				ofp.println ();
				ofp.print (
				comment_space + _argv[i] + " " );
				column = column0 + len + 1;
			}
			else {	// Put the argument on the same line...
				ofp.print ( _argv[i] + " " );
				column += (len + 1);
			}
		}
	...
	if ( _command_list != null ) {
		...
		for ( i = 0; i < size; i++ ) {
			ofp.println ( comment + " " +
			(String)_command_list.elementAt(i) );
		}
	}
		...
		try {	file = new FileReader ( _command_file );
...
{	String		comment;
	...
	if ( oldfile == null ) {
		if ( Message.isDebugOn ) {
			Message.printDebug ( dl, routine,
			"NULL old file - no old header" );
		}
		oldheader = null;
	}
	else if ( oldfile.length() == 0 ) {
		Message.printWarning ( dl, routine,
		"Empty old file - no old header" );
		oldheader = null;
	}
	else {	// Try to get the header...
		oldheader = getFileHeader (	oldfile, comment_strings,
						ignore_comment_strings, 0 );
		if ( oldheader != null )
	...
	try {	ofp = new PrintWriter ( new FileOutputStream(newfile) );
		...
		e.printStackTrace();
		Message.printWarning ( wl, routine,
		"Unable to open output file \"" + newfile + "\"" );
		return null;
	}

	// Print the new file header.  If a comment string is not specified, use
	// the default...

	if ( (comment_strings == null) || (comment_strings.length == 0) ) {
		comment = UNIVERSAL_COMMENT_STRING;
	}
	else {	comment = comment_strings[0];
	}
...
interfaces, immediately before processing commands.
