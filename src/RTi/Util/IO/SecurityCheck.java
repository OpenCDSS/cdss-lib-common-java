...
/**
This class provides some general capabilities to check security settings
in a way that is simpler than the Java SecurityManager calls.
These static functions can be called to determine whether a file can be written, printed, etc.
...
public static boolean canExec ( String command0 ) {
	...
	if ( command0 == null ) {
		// Use a default file for testing.  First figure out if are on UNIX or not.
...
public static boolean canReadFile ( String file0 ) {
	...
	// See if can read a file.

	if ( file0 == null ) {
		// Use a default file for testing.  First figure out if we are on UNIX or not.
		if ( IOUtil.isUNIXMachine() ) {
...
// canWriteFile - see if can write to the local system...
//
