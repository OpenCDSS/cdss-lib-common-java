...
*/
public abstract
class AbstractCommand extends Object implements Command, CommandStatusProvider
{
...
/**
The command processor, which will run the command and be associated with the
command GUI.
*/
...
This is initialized to an empty PropList and should be set when initializing the
command.
*/
...
*/
public Object clone ()
{	try {	AbstractCommand command = (AbstractCommand)super.clone();
...
{	String routine = "SkeletonCommand.parseCommand", message;
	...
	if ( tokens.size() > 1 ) {
		...
		try {	__parameters = PropList.parse ( Prop.SET_FROM_PERSISTENT,
...
been parsed.
@param parameters The command parameters as a PropList - only String parameter
