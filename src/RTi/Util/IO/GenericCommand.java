...
/**
This class is a generic command, for example to use when editing a command that does not have a command editor.
...
*/
public void initializeCommand ( String command, CommandProcessor processor, boolean fullInitialization )
throws InvalidCommandSyntaxException, InvalidCommandParameterException {
	// Save the processor.
	super.initializeCommand ( command, processor, fullInitialization );
...
/**
Run the command.
@param commandNumber the command number (1+) being processed
@exception CommandWarningException Thrown if non-fatal warnings occur (the command could produce some results).
@exception CommandException Thrown if fatal warnings occur (the command could not produce output).
*/
public void runCommand ( int commandNumber )
throws InvalidCommandParameterException, CommandWarningException, CommandException {
	// Throw an exception because if something tries to run with this it needs to be made known that nothing is happening.
...
// TODO SAM 2005-05-31 If the editor is ever implemented with a tabular display for parameters,
// will need to deal with the parsing.
