...
/**
The DataType class provides capabilities for reading and storing data type information.
Data types are used to look up the dimension, default units, and SHEF (Standard Hydrologic Exchange Format) information.
Data types are maintained internally using a Vector of DataType (self-referencing).
Data types are typically read from a persistent source at application startup.
*/
...
/**
Default English units (e.g., when creating a new time series in a system configured for English units. "DEGF" for NWSRFS temperature).
*/
...
/**
Either "OBS" for observed or "SIM" for simulated.
This is a hold-over from NWSRFS conventions, which limit some data types' use to only observed or simulated data.
*/
...
/**
SHEF (Standard Hydrologic Exchange Format) physical element data type.  The value is populated when SHEF data are read.
@see SHEFType
*/
...
/**
Add a data type to the internal list of data types.  After adding, the data type can be used throughout an application.
...
public static List<DataUnit> lookupUnitsForDimension ( String system, String dimension ) {
	...
	for ( int i = 0; i < size; i++ ) {
		...
		if ( (system != null) && !system.equals("") &&
			...
			// The system does not equal the requested value so ignore the units object (system of "" is OK for ENGL and SI).
			continue;
		}
...
/**
Read a file that is in NWS DATATYPE format.  See the fully loaded method for more information.
...
/**
Read a file that is in NWS DATATYPE format.
The NWS DATAUNIT file should be read before calling this method, using DataUnits.readNWSUnitsFile().
This routine depends on on the values in the DATATYPE file originally supplied by the NWS.
The file is normally named /awips/hydroapps/lx/rfc/nwsrfs/sys_files/DATAUNIT and has the format:
<p>
<pre>
*   04/19/2000   DATATYPE
*
...
In this format, multiple lines are used for each data type,
with the data type abbreviation being included at the start of each line.
The first line includes data type, and description, possibly for 3 levels of output
(when read here, the end of the line is set to the description, removing extra whitespace).
The second line includes data type, dimension, whether an areal ("A"),
point ("P") or both ("A/P") measurement, the time scale ("ACCM", "MEAN", or "INST",
whether used in the forecast and calibration system ("CALB", "FCST",
or "BOTH", ignored), number of data values per time step (ignored),
and whether the data can be stored in DATACARD format ("YES" or "NO", ignored).
If the data type is used in the forecast system, a line will be included for forecast system information, including data type,
...
("YES" or "NO", ignored), the code for the component that can write the data type
("PP" for preprocessor or "FC" for forecast, ignored), and the number additional pieces of information (ignored).
If the data type is used in the calibration system, a line will be included for calibration system information,
including data type, the word "CALB".
...
This is required in many cases because defining a data type instance checks the dimension against defined dimensions.
*/
...
throws IOException {
	...
	try {	// Main try.
	// Open the file (allow the data type file to be a normal file or a URL so web applications can also be supported).
	try {
	...
	int linecount = 0;
	DataType type = null;
	String type_prev = "";	// Previous type, to keep track of how many lines are read for the same type.
	int type_count = 0;	// Count of how many times the type has been read, to keep track of multiple lines of input.
	...
	String where_used = null;	// Indicates whether the data type is used in the calibration and/or forecast system.
	while ( true ) {
		// Read a line.
		...
		if ( type_count == 1 ) {
			...
			// The description is comprised of the Level 1, 2, 3 descriptions.  Sometimes these have a "  ." at the end so remove.
	...
	fp.close();
	// Add FMAP as a data type.  This is not in the DATAUNIT file because it is considered the future part of MAP.
	// However, it has special meaning in the database files so treat as a special case.
...
This routine depends on on the values in an RTi DATAUNIT file.
...
throws IOException {
	...
	for ( int i = 0; i < nstrings; i++ ) {
		...
		if ( define_dimensions ) {
			...
			// data so that it can be referenced below.
			// It is OK to define more than once because DataDimension will keep only one unique definition.
		...
		units.setOutputPrecision ( StringUtil.atoi( ((String)tokens.elementAt(5)).trim()) );
		units.setMultFactor ( StringUtil.atod( ((String)tokens.elementAt(6)).trim()) );
		units.setAddFactor ( StringUtil.atod( ((String)tokens.elementAt(7)).trim()) );
		...
		addUnits ( units );
			...
			Message.printWarning ( 2, routine, "Error reading units at line " + (i + 1) + " of file \"" + dfile + "\"" );
		}
	}
...
@param abbreviation Data type abbreviation (e.g., "MAP" for mean areal precipitation).
*/
