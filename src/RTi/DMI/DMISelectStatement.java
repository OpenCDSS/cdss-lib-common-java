...
/**
The DMISelectStatement class stores basic information about SQL select statements,
allowing select statements to be formed and executed.
...
and is mainly being implemented to support SQL Server, although some other engines are checked for in toString().
*/
...
/**
Executes this statement's stored procedure to query data.
If this statement was not set up as a Stored Procedure, an exception will be thrown.
...
throws SQLException {
	...
	return this.__storedProcedureCallableStatement.executeQuery();
}
...
private String toAccessString() {
	...
	int size = this._field_Vector.size();
	if (size > 0) {
		statement.append ( this._field_Vector.get(0) );
		for ( int i = 1; i < size; i++ ) {
			statement.append ( ", " + this._field_Vector.get(i) );
		}
	}

	size = this._table_Vector.size();
	if (size > 0 && this._join_Vector.size() == 0) {
		statement.append ( " FROM " );
		statement.append ( this._table_Vector.get(0) );
		for ( int i = 1; i < size; i++ ) {
			statement.append ( ", " + this._table_Vector.get(i) );
		}
	}

	size = this._join_Vector.size();
	if (size > 0) {
		int type = -1;
		String s = null;
		...
		for (int i = 0; i < size; i++) {
			type = this._join_type_Vector.get(i).intValue();
			if (type == _JOIN_INNER) {
			...
			s = this._join_Vector.get(i);
			statement.append(s + " ON ");
			s = this._on_Vector.get(i);
			statement.append(s);
	...
	size = this._where_Vector.size();
	if (size > 0) {
		statement.append ( " WHERE " );
		statement.append ( this._where_Vector.get(0) );
		for ( int i = 1; i < size; i++ ) {
			statement.append ( " AND (" + _where_Vector.get(i) + ")");
		}
	}

	size = this._order_by_Vector.size();
	if (size > 0) {
		if (this._groupBy) {
		...
		statement.append ( this._order_by_Vector.get(0) );
		for ( int i = 1; i < size; i++ ) {
			statement.append ( ", " + this._order_by_Vector.get(i));
		}
	}
...
public String toString() {
	...
	if (this._dmi.getDatabaseEngine().equalsIgnoreCase("Access")) {
    ...
    if ( this._top > 0 ) {
        if ( this._dmi.getDatabaseEngineType() == DMIDatabaseType.SQLSERVER ) {
            statement.append ( "TOP " + this._top + " " );
        }
    }
	...
	if (size > 0) {
		statement.append ( this._field_Vector.get(0) );
		for ( int i = 1; i < size; i++ ) {
	        statement.append ( ", " + DMIUtil.escapeField(this._dmi,this._field_Vector.get(i)) );
		}
	}
	...
	int whereSize = this._where_Vector.size();
	if (whereSize > 0) {
		statement.append ( " WHERE " );
		statement.append ( this._where_Vector.get(0) );
		for ( int i = 1; i < whereSize; i++ ) {
			statement.append ( " AND (" + this._where_Vector.get(i) + ")");
		}
	}
    if ( this._top > 0 ) {
        if ( this._dmi.getDatabaseEngineType() == DMIDatabaseType.ORACLE ) {
            if ( whereSize == 0 ) {
                statement.append ( " WHERE " );
            }
            else {
            ...
            statement.append ( " (ROWNUM <= " + this._top + ")" );
        }
    }
    ...
    if ( this._top > 0 ) {
        if ( this._dmi.getDatabaseEngineType() == DMIDatabaseType.MYSQL ) {
            statement.append ( " LIMIT " + this._top );
        }
    }

	// Add the trailing character to the statement, may be empty string.
	String end = this._dmi.getStatementEnd();
