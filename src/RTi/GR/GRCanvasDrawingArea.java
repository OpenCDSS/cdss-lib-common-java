...
public GRCanvasDrawingArea () {
	super ();
	String routine = getClass().getSimpleName() + ".GRCanvasDrawingArea";
...
*/
public GRCanvasDrawingArea ( GRCanvasDevice dev, String name, GRAspectType aspect,
...
@param y Y-coordinate of center.
@param ry Y-radius.
@param fillmode the fill type (currently ignored)
*/
...
public void fillPolygon ( int npts, double x[], double y[], int transparency ) {
	...
	int height = hiY - loY;
	if ( (width == 0) || (height == 0) ) {
		// Basically a line - just draw normal to avoid division by zero elsewhere.
		_jdev._graphics.fillPolygon ( ix, iy, npts );
		return;
	}
	// Some degree of transparency.

	// 2. Create the off-screen image based on the extents calculated in step 1.

	// Create a Canvas object in order to use its "createImage" method to create an off-screen bufferable Image.
	// In fact, any object that extends "Component" could be used here.
	//Canvas tempCanvas = new Canvas();
	//Image buffer = tempCanvas.createImage(width, height);
	// This step - don't we need the image from the original off-screen buffer.
	// Otherwise, how do the pixels "show through" in the image that is drawn in the last step?
	Image buffer = _jdev.createImage(width, height);

	// 3. Translate the x and y coordinates of the Polygon in order to draw it inside the buffer Image object.
	// tx will contain the translated x points, and ty will contain the translated y points.
	...
	//    If not, their alpha value gets set to the 'alpha' variable passed into this method.
	int numPixels = width * height;

	// Calculate the integer value of the bitwise Or of the Kcolor's color values.
	// This will be used to check for which pixels should be set to transparent alphas in the final image.
	// These pixels will then not be transferred.

	int K = (Kcolor.getRGB() & 0x00ffffff);
	int a = 0;
	for (int i = 0; i < numPixels; i++) {
		// If the non-alpha portion of the pixel is equal to the K value,
		// this pixel will be totally transparent and will not be transferred to the final image.
	...
	//	Transparent pixels in the image do not affect the existing pixels.
...
public void fillRectangle (	GRLimits limits ) {
	...
	if ( Message.isDebugOn ) {
		String routine = getClass().getSimpleName() + ".fillRectangle(GRLimits)";
...
public void fillRectangle (	double xll, double yll, double width, double height ) {
	if ( Message.isDebugOn ) {
		String routine = getClass().getSimpleName() + ".fillRectangle(x,y,w,h)";
...
public GRLimits getDataExtents ( GRLimits limits, int flag ) {
	...
	GRPoint xymin = getDataXY ( ((_plotx2 + _plotx1)/2.0 -
			limits.getWidth()/2.0),
			((_ploty2 + _ploty1)/2.0 -
			limits.getHeight()/2.0), GRCoordinateType.PLOT );
...
Currently, device units (GRUnits.DEV) are always returned and the GR.getTextExtents call scales to the device.
REVISIT (JTS - 2003-05-05)
This parameter isn't even used
SAM:
...
public void setFont ( String name, int style, int size ) {
	...
	//Message.printStatus ( 1, "da.setFont", "Set font to " + name + " size " + size );
