...
/**
This class stores a two-dimensional regular or irregular grid.
All grid lines are parallel in the data's projection but the distance between grid lines does not need to be constant.
This class can be used by GIS or other classes to store gridded data but this
class only saves information about the grid layout, not the data itself.
The grid always represents coordinates with the origin at the lower left, as shown in the following figure.
Because grid files can contain more data than is necessary for viewing/processing,
the class allows active minimum and maximum coordinates to be set.
These coordinates are for the lower-left and the upper-right cells.
Note that when identifying cells,
the cell should be identified by the row and column coordinates of the cell (not the edge).

The full grid is used with *Full() methods.  Otherwise, the methods use the active grid.
The coordinates of row and column edges are defined by floating
point values and can be set by using the GRShape base class xmin, ymin, xmax, ymax data members.
Methods to set edge values for irregular data have not yet been implemented.
<pre>
                      +----+----+----+----+----+----+
                      |    |    |    |    |    |    |
                      +----+----+----+----+----+----+
...
Currently this class includes functionality only for regular grids and only integer coordinates are used.
*/
...
@return true if the indicated column and row are in the active grid, false if not.
*/
...
/**
Return a polygon of 5 points to represent a a single grid cell.
The order of points in the polygon is clockwise, starting in the SouthWest corner
(southwest, northwest, northeast, southeast, southwest).
The dimension of the cell is controlled by the grid dimensions.  The units are those of the grid data.
...
/**
Set the active size of the grid.
Call this method after setting the minimum X and Y coordinates.
...
@param max_row Top-most row.
*/
public void setSizeFull ( int min_column, int min_row, int max_column, int max_row ) {
