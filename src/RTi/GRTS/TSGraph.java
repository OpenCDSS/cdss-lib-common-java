...
which was for the original time series list (not the subset used just for this
graph).
*/
public TSGraph (	TSGraphJComponent dev, GRLimits drawlim_page,
			TSProduct tsproduct, PropList display_props,
			int subproduct, Vector tslist,
			int reference_ts_index ) {
	...
	if ( tslist == null ) {
		// Create an empty vector so checks for null don't need to be
		// added everywhere...
		__tslist = new Vector(1);
	}
	else {	__tslist = tslist;
	}
	...
	if (Message.isDebugOn) {
		// Might need to use this when we try to process all null time
		// series...
	...
	_graph_type = TSProduct.lookupGraphTypeNumber (
			tsproduct.getLayeredPropValue ( "GraphType", subproduct,
			-1, false ) );
	if ( _graph_type < 0 ) {
		// Should never happen...
	...
	// This is also checked in the paint() method in case any analysis
	// settings change...
	...
	else {	_da_graph.setDataLimits ( _data_limits );
	}
	...
	if ( __left_tslist != null ) {
		...
		for ( int i = 0; i < size; i++ ) {
			...
			// Check the interval so that we can make decisions
			// during plotting...
			try {	_interval_max = MathUtil.max (
						_interval_max,
						ts.getDataIntervalBase() );
				_interval_min = MathUtil.min ( _interval_min,
						ts.getDataIntervalBase() );
			}
			catch ( Exception e ) {
...
public void actionPerformed(ActionEvent event) {
	...
	if ( command.equals(__MENU_PROPERTIES) ) {
		...
		TSProductJFrame pframe =
			frame.getTSViewJFrame().getTSProductJFrame();
		...
		if (_graph_type 
			...
			for (int i = 0; i < nreg; i++) {
				...
				regressionData = (TSRegression)_regression_data
					.elementAt(i);
		...
		try {
			...
			Message.printWarning(2, routine, 
				"There was an error getting the limits for "
		...
		if (IOUtil.testing()) {
			Message.printStatus(1, "", "Start: " + _start_date);
			Message.printStatus(1, "", "  End: " + _end_date);
			Message.printStatus(1, "", "  Max: " 
				+ limits.getMaxValue());
			Message.printStatus(1, "", "  Min: " 
				+ limits.getMinValue());
		}

		pframe.setMaximumYValue("" + limits.getMaxValue(),
			true);
	}
		...
		pframe.setMinimumYValue("" + limits.getMinValue(),
			true);
	}	
...
{	// First figure out if the slope of the line is up to the right or
	// down to the right...
	int i = 0;
	double x_edge = 0.0;
	...
	if ( y[0] < y[npts - 1] ) {
		...
		// Adjust the points on the left...
		for ( i = 0; i < npts; i++ ) {
			if ( y[i] >= min_datay ) {
				// Found the first Y that is on the graph.
				// Interpolate the previous point...
				if ( i == 0 ) {
					// No need to do more...
					break;
				}
				x_edge = MathUtil.interpolate ( min_datay,
					y[i - 1], y[i], x[i - 1], x[i] );
				x[i - 1] = x_edge;
				y[i - 1] = min_datay;
				// Now set all previous points to the
				// interpolated edge value.
		...
		for ( i = (npts - 1); i >= 0; i-- ) {
			if ( y[i] <= max_datay ) {
				...
				x_edge = MathUtil.interpolate ( max_datay,
					y[i + 1], y[i], x[i + 1], x[i] );
				x[i + 1] = x_edge;
				y[i + 1] = max_datay;
				// Now set all previous points to the
	...
	else {	// Slope is down to the right...
		// SAMX ... do later... inverse correlation not likely with
		// what we are doing. 
	}
}
...
only applies currently to empty graphs.  For other graphs, see 
setComputeWithSetDates().
*/
protected void computeDataLimits (boolean max)
// JTS
{	String routine = "TSGraph.computeDataLimits";
	// JTS
	// exceptions thrown when trying to draw empty graph
		...
		if (max) {
			if (_max_start_date == null) {
				...
				_data_limits = new GRLimits(
					_max_start_date.toDouble(), 0, 
					_max_end_date.toDouble(), 1);
				_max_data_limits = new GRLimits(_data_limits);
			}
		}
		else {
			if (_start_date == null) {
				...
				_data_limits = new GRLimits(
					_start_date.toDouble(), 0,
					_end_date.toDouble(), 1);
				_max_data_limits = new GRLimits(_data_limits);
			}
		}
		return;
	...
	try {	// First get the date limits from the full set of time series...
		TSLimits limits = null;
		if (_graph_type 
		    == TSProduct.GRAPH_TYPE_PREDICTED_VALUE_RESIDUAL) {
	    		int nreg = 0;
			...
			limits = TSUtil.getPeriodFromTS(getEnabledTSList(), 
				TSUtil.MAX_POR);
		}
		...
		// Now get the data limits.  To do the check correctly, the data
		...
		// Now check the property (keep a separate copy so we can avoid
		// the prompt below if appropriate)...
		String ignore_units_prop =
				_tsproduct.getLayeredPropValue (
				"LeftYAxisIgnoreUnits", _subproduct, -1, false);
		boolean ignore = false;
		if (	(ignore_units_prop != null) &&
			ignore_units_prop.equalsIgnoreCase("true") ) {
			_ignore_units = true;
			if (TSUtil.areUnitsCompatible(getEnabledTSList(),
			    true)) {
				_ignore_units = false;
				_tsproduct.setPropValue("LeftYAxisIgnoreUnits", 
					"false", _subproduct, -1);
				ignore = true;
			}
		}
		try {	
			if (_ignore_units) {
				// Can ignore units...
				_max_tslimits = TSUtil.getDataLimits(
					getEnabledTSList(),
					_start_date, _end_date, "", false,
					_ignore_units);
			}
			else {	// Need to have consistent units.  For now
				// require them to be the same because we don't
				// want to do units conversions on the fly or
				// alter the original data...
				//
				// SAMX - need to add on the fly conversion of
				// units (slower but changing original data is
				// a worse alternative).
				if (!TSUtil.areUnitsCompatible(
				    getEnabledTSList(), true)) {
					if (_is_reference_graph) {
						// Rely on the main graph to set
						// the _ignore_units flag and
						// determine whether the graph
						// view needs to be closed.
						// Assume
						// that the _ignore_units flag
						// can be set to true since
						// the reference graph only
						// displays one graph.
						_ignore_units = true;
						_max_tslimits = TSUtil
							.getDataLimits(
							__tslist, _start_date,
							_end_date, "", false,
							_ignore_units);
					}
					else if (ignore) {
						_max_tslimits =
							TSUtil.getDataLimits(
							getEnabledTSList(), 
							_start_date,
							_end_date, "", false,
							_ignore_units);
						...
						else {	// No frame so default
							// to ignore units...
							x = ResponseJDialog.YES;
						}
						if ( x == ResponseJDialog.NO ) {
							// Set this so that code
							// that uses this
							// component can check
							// to see if the
							// component needs
	...
	if (_graph_type == TSProduct.GRAPH_TYPE_PREDICTED_VALUE_RESIDUAL) {
		...
		for (int i = 0; i < nreg; i++) {
			...
			regressionData = (TSRegression)_regression_data
				.elementAt(i);
			v.add(regressionData.getResidualTS());
			tempLimits = TSUtil.getDataLimits(v, _start_date,
		...
		_max_tslimits = TSUtil.getDataLimits(getEnabledTSList(), 
			...
			if ( _graph_type == TSProduct.GRAPH_TYPE_PERIOD ) {
				...
				_max_tslimits.setMinValue(
					getEnabledTSList().size() + 1);
			}
		}
		catch ( Exception e ) {
			// This typically throws an exception if the data
			// are not of consistent units.
			if ( !_is_reference_graph ) {
				Message.printWarning ( 1, routine, "Data are " +
				"not compatible (units?).  Cannot graph." );
				Message.printWarning ( 2, routine, e );
			}
		}
		if ( _max_tslimits == null ) {
			// Typically due to a cancel of the graph due to
			// incompatible units.  In this case we get to here but
			// just need to gracefully handle nulls until the graph
			// can be closed in parent container code...
			return;
		}
		if (_is_reference_graph && (_reference_ts_index >= 0)) {
			// Reset the coordinates based only on the reference
			// time series but use the full period for dates...
			Vector ref_tslist = new Vector(1,1);
			ref_tslist.addElement((TS)__tslist.elementAt(
				_reference_ts_index));
			TSLimits reflimits = TSUtil.getDataLimits (
				ref_tslist, _start_date, _end_date, "", false,
				_ignore_units );
			...
			if ( Message.isDebugOn ) {
				...
				_gtype +
				"Reference graph max data limits are " +
				_max_tslimits.toString());
			}
		}
		else {	if ( Message.isDebugOn ) {
				Message.printDebug ( 1, routine,
				_gtype + "Main graph max data limits are " +
				_max_tslimits.toString());
			}
			// If the properties are given, set the limits to the
			// given properties, but only if they are outside the
			// range of the data that was determined...
			//
			// REVISIT SAM 2006-09-28
			// Still need to evaluate how switching between Auto
			// and hard limits can be handled better.
			String prop_value = _tsproduct.getLayeredPropValue (
					"LeftYAxisMax", _subproduct, -1, false);
			if (	(prop_value != null) &&
				StringUtil.isDouble(prop_value) ) {
				double ymax = StringUtil.atod(prop_value);
				if (!_zoom_keep_y_limits
				    && ymax > _max_tslimits.getMaxValue()) {
					_max_tslimits.setMaxValue(ymax);
				}
			}
			else if (prop_value != null 
			    && prop_value.equalsIgnoreCase("Auto")) {
///////////////////
// indents removed for legibility
	if (_graph_type == TSProduct.GRAPH_TYPE_PREDICTED_VALUE_RESIDUAL) {
		int nreg = 0;
		if (__tslist != null) {
			nreg = __tslist.size() - 1;
		...
		for (int i = 0; i < nreg; i++) {
			...
			regressionData = (TSRegression)_regression_data
				.elementAt(i);
			v.add(regressionData.getResidualTS());
			tempLimits = TSUtil.getDataLimits(v, _max_start_date,
				_max_end_date, "", false, _ignore_units );
			if (tempLimits.getMaxValue() > maxValue) {
				maxValue = tempLimits.getMaxValue();
			}
			if (tempLimits.getMinValue() < minValue) {
		...
		TSLimits tempLimits = TSUtil.getDataLimits(getEnabledTSList(), 
			_max_start_date, _max_end_date, "", false, 
			_ignore_units);
...
///////////////////
			...
			prop_value = _tsproduct.getLayeredPropValue (
					"LeftYAxisMin", _subproduct, -1, false);
			if (	(prop_value != null) &&
				StringUtil.isDouble(prop_value) ) {
				double ymin = StringUtil.atod(prop_value);
				if (!_zoom_keep_y_limits
				    && ymin < _max_tslimits.getMinValue()) {
		...
		if ( _graph_type == TSProduct.GRAPH_TYPE_XY_SCATTER ) {
			...
			for ( int ir = 0; ir < nregression; ir++ ) {
				...
				if (	(regressionData != null) &&
					!regressionData.isMonthlyAnalysis() ) {
					// One equation...
					if ( regressionData.isAnalyzed() ) {
						// Analysis was successful so
						// full data are available...
						//
						// Get the limits from the
						// regression data since some
						// data may have been ignored...
						// Y is the 2nd+ time series
						// picked.
						if ( xlimits_found ) {
							xmin =
							MathUtil.min(xmin,
						...
						// Regression was not
						// successful but if no line was
						// requested, still plot the
						// points...  special case found
						// in snow GUI.
						/* REVISIT - for now do
						...
						"SAMX",
						"No line but number of"+
						String prop_val = _tsproduct.
							getLayeredPropValue (
							"RegressionLineEnabled",
							_subproduct, ir,false);
						if (	(prop_val != null) &&
							!prop_val.
							equalsIgnoreCase(
							"true") ) {
						...
						*/
						...
						// common limits, but that may
						// not be available - need to
						// do some work in TSRegression.
						TS rts= regressionData.
							getIndependentTS();
						TSLimits lim =
							rts.getDataLimits();
						if (	(lim != null) &&
				...
				else if ((regressionData != null) &&
					regressionData.isMonthlyAnalysis() ) {
					boolean [] analyze_month =
					regressionData.getAnalyzeMonth();
					// Monthly analysis...
					//
					// Get the limits from the regression
					// data since some data may have been
					// ignored...
					...
					if ( xlimits_found ) {
						xmin = MathUtil.min(xmin,
						regressionData.getMinX1(il));
						xmax = MathUtil.max(xmax,
						regressionData.getMaxX1(il));
					}
					else {	xmin=
						regressionData.getMinX1(il);
						xmax=
					...
					if ( ylimits_found ) {
						ymin = MathUtil.min(ymin,
						regressionData.getMinY1(il));
						ymax = MathUtil.max(ymax,
						regressionData.getMaxY1(il));
					}
					else {	ymin=
						regressionData.getMinY1(il);
						ymax=
			...
			if (!ylimits_found) {
				// Loop through the Y axis time series
				// (hopefully this code is never executed).
				TS ts = null;
				...
				for ( int its = 1; its <= nregression; its++ ) {
					...
					if ( ylimits_found ) {
						ymin = MathUtil.min ( ymin,
							ylimits.getMinValue() );
						ymax = MathUtil.min ( ymax,
							ylimits.getMaxValue() );
					}
					else {	ymin = ylimits.getMinValue();
			...
			// Set the limits regardless.  Worst case they will
			// be zero to one...
			_data_limits = new GRLimits ( xmin, ymin, xmax, ymax );
		}
		else if ( _graph_type == TSProduct.GRAPH_TYPE_DURATION ) {
			// X limits are 0 to 100.  Y limits are based on the
			// time series...
			_data_limits = new GRLimits (
					0.0, 0.0,
					100.0, _tslimits.getMaxValue() );
		}
		else if (_graph_type 
		    == TSProduct.GRAPH_TYPE_PREDICTED_VALUE_RESIDUAL
			...
			for (int ir = 0; ir < nregression; ir++) {
				...
				if (regressionData != null) {
					...
					tsLimits = TSUtil.getDataLimits(
						tempV, _max_start_date,
						_max_end_date, "", false,
						_ignore_units);
				}
				else {
				...
				if (tsLimits != null) {
					if (tsLimits.getMaxValue() > maxValue) {
						maxValue 
						       = tsLimits.getMaxValue();
					}
					if (tsLimits.getMinValue() < minValue) {
						minValue 
						       = tsLimits.getMinValue();
					}
				}
			}

			_data_limits = new GRLimits (
				_start_date.toDouble(), minValue, 
				_end_date.toDouble(), maxValue);
		}
		else {	
			_data_limits = new GRLimits (
				_start_date.toDouble(),
				_tslimits.getMinValue(),
				_end_date.toDouble(),
				_tslimits.getMaxValue() );
		}
		...
		if (_data_limits != null) {
			if (max) {
				...
				if (_graph_type 
				    == TSProduct.GRAPH_TYPE_XY_SCATTER) {
				    	_max_data_limits = new GRLimits(
						_data_limits);
				}
				else {
				GRLimits tempLimits = new GRLimits(
					_max_start_date.toDouble(),
					_tslimits.getMinValue(),
					_max_end_date.toDouble(),
					_tslimits.getMaxValue() );
			...
			if (Message.isDebugOn) {
				Message.printDebug ( 1, routine, _gtype
					+ "Initial computed _max_data_limits "
		...
		Message.printWarning(2, routine,
			_gtype + "Error getting dates for plot.");
...
public Vector getEnabledTSList() {
	...
	Vector v = new Vector();
	for (int i = 0; i < size; i++) {
		propValue = _tsproduct.getLayeredPropValue("Enabled",
			_subproduct, i, false);
