...
@param classification Class for legend (zero-index).
Ignored for single classification.  This is the position in the color table.
*/
public GeoLayerViewLegendJComponent(GeoLayerView layer_view, int isym, int classification) {
	...
	if ( sym.getShapeType() == GRSymbolShapeType.VERTICAL_BAR_SIGNED ) {
		// Make the symbol be as large as the twice the vertical height so positive and negative values can be shown to the full scale.
	...
	else if (sym.getShapeType() == GRSymbolShapeType.TEACUP) {
		sym_size = 20;
	...
	_layer_grda = new GRJComponentDrawingArea ( this, "", GRAspectType.TRUE,
...
private void drawSymbol () {
	...
	// Set data limits to the drawing limits.
	// That way can make sure that the symbol position can be controlled.
	_layer_grda.setDataLimits ( limits );
	// Fill the full drawing area with the background color.
	// Can only do after the canvas and drawing area are fully defined.
	GRSymbol sym = null;
	GRColor color = null;
	boolean dodraw = true;
	try {
		...
		else if ( sym.getClassificationType() == GRClassificationType.SCALED_SYMBOL ) {
			color = sym.getColor();
		}
		...
		if ( dodraw && ((layerType == GeoLayer.POINT) || (layerType == GeoLayer.POINT_ZM) ||
			...
			if ( sym.getClassificationType() == GRClassificationType.SCALED_SYMBOL ) {
				if ( sym.getShapeType() == GRSymbolShapeType.VERTICAL_BAR_SIGNED ) {
					...
					sym_data[0] = 1.0;
					GRDrawingAreaUtil.drawSymbol( _layer_grda, sym.getShapeType(),
					...
					// Since unsigned bars only show positive values only get the first color to use to draw the bar.
					color = sym.getColor();
					double [] sym_data = new double[1];
					sym_data[0] = 1.0;
					GRDrawingAreaUtil.drawSymbol(
						_layer_grda, sym.getShapeType(),
						(limits.getLeftX() + sym.getSize()*.5 + 1.0),
						limits.getBottomY(),
						sym.getSizeX(),
						sym.getSizeY(), 0.0, 0.0,
						sym_data, GRUnits.DEVICE,
						GRSymbolPosition.CENTER_X | GRSymbolPosition.BOTTOM);
				}
			}
			else {
				// A simple symbol.
