...
/**
Ghe GeoViewZoomHistory class saves data and device limits from zoom events in a GeoView.
Any application using this class must implement GeoViewListener and add the geoViewListener to the desired GeoView.
The application must instantiate a GeoViewZoomHistory and add to the zoom history in the following manner:
<p>
...
/**
Save the zoom limits at the current index.
Therefore, if the current index is not at the end of the zoom history,
this set of limits will be inserted in the history at the appropriate location.
...
public void addHistory ( GRLimits devLimits, GRLimits dataLimits ) {
	...
	if ( _currentIndex == _dataLimitsHistory.size()) {
		...
		_dataLimitsHistory.add( _currentIndex, dataLimits );
	}
	...
	if ( _currentIndex == _devLimitsHistory.size()) {
		...
		_devLimitsHistory.add ( _currentIndex, devLimits );
	}
}
...
throws Exception {
	if ( (index < 0) || (index >= _dataLimitsHistory.size()) ) {
		throw new Exception ( "Index " + index + " out of bounds (" + _dataLimitsHistory.size() + ")");
	}
...
/**
Retrieve the next data limits if available and increment the current index if changeCurrentIndex is true.
...
/**
Retrieve the previous data limits if available and decrements the current index if changeCurrentIndex is true.
